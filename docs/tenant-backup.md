...
(schema version 2): the tenant's `catalog_resources` registry. It sits _beside_
the ZIPs rather than inside them
...
Without it a restore quietly breaks the `template_resource_id` on generation
history: no foreign key protects it, so a dangling one simply stops joining. A
schema-1 archive has no `identities.json` and restores with fresh identities,
exactly as it did when it was written; an `aliases` list written by a
pre-release development build is ignored.
