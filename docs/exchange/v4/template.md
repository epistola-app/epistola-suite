...
- `templateModel` is the editor model; its own evolution is tracked by the embedded `modelVersion`, independent of this wire contract. This is the history of this resource's shape; on the wire it carries the catalog-wide `schemaVersion` (currently `4`), not an independent template version.
