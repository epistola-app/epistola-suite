...
**Stability contract:** at 1.0 the wire format is **`schemaVersion 4`**, **versioned and append-only** — additive (optional-field) changes stay within a version, and any non-round-trip-compatible change bumps the version and ships a migration step. What a consumer can rely on across Epistola versions, and exactly what counts as additive vs. breaking, is in [stability.md](stability.md).
