...
- Code lists became the canonical home for enumerations when the [attribute](attribute.md) shape moved from inline `allowedValues` to a `codeListBinding`. This is the history of this resource's shape; on the wire it carries the catalog-wide `schemaVersion` (currently `5`), not an independent code-list version.
