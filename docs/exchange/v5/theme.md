...
- `documentStyles.fontFamily` is the structured `{ slug, catalogKey? }` font reference (not a CSS stack). This is the history of this resource's shape; on the wire it carries the catalog-wide `schemaVersion` (currently `5`), not an independent theme version.
