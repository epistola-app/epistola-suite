...
- A stencil node uses `props.version` for its exact published base and
  `props.draftVersion` for its exact mutable draft. `draftVersion` is absent
  from publishable catalog content. The removed v4 `props.isDraft` boolean is
  rejected in v5.
