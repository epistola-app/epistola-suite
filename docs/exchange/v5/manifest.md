...
- Catalog versioning landed the `release.fingerprint` field, enabling content-based change detection (see `CatalogConstants.kt` header note). This is the history of the manifest's shape; on the wire it always carries the catalog-wide `schemaVersion` (currently `5`), not an independent manifest version.
