...
hand-rolled-HTMX conversion), not a shared fetch layer. Only genuinely client-side
...
requests against fragment endpoints (form-encoded, see the hand-rolled-HTMX
conversion), and `api-client.js` was deleted rather than adopted. The remaining legitimate
`fetch()` sites (editor callbacks, blob previews) handle their responses inline.
