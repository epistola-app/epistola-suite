...
- **Propagate tenant_key**: Every table referencing a tenant-scoped entity gets its own `tenant_key` column for composite FKs
- **Domain models**: Only add `tenantId` to Kotlin models where it wasn't already present (`TemplateVariant`). `TemplateVersion`, `VersionSummary`, and `VariantSummary` do NOT need it — tenant_key is always known from context and doesn't need to travel through the model. The DB column exists for FK integrity, not application logic.
- **Simpler queries**: Many JOINs that existed solely for tenant isolation (e.g., `JOIN document_templates dt ON ... WHERE dt.tenant_key = ...`) become direct WHERE clauses on the table's own `tenant_key` column.
...
CREATE INDEX idx_document_templates_tenant_key ON document_templates(tenant_key);
...
-- Drop idx_document_templates_tenant_key (tenant_key is now the leading PK column)
```
...
-- Drop idx_environments_tenant_key (tenant_key is now the leading PK column)
```
...
CREATE TABLE template_variants (
    id VARCHAR(50) PRIMARY KEY CHECK (...),
    template_key VARCHAR(50) NOT NULL REFERENCES document_templates(id) ON DELETE CASCADE,
...
CREATE INDEX idx_template_variants_template ON template_variants(tenant_key, template_key);
```
...
CREATE TABLE template_versions (
    id INTEGER NOT NULL,
    variant_key VARCHAR(50) NOT NULL REFERENCES template_variants(id) ON DELETE CASCADE,
    ...
    PRIMARY KEY (variant_key, id),
);
...
CREATE TABLE template_versions (
    ...
    ...
    PRIMARY KEY (tenant_key, variant_key, id),
...
CREATE INDEX idx_template_versions_variant ON template_versions(tenant_key, variant_key);
CREATE INDEX idx_template_versions_status ON template_versions(status);

CREATE UNIQUE INDEX idx_one_draft_per_variant
    ON template_versions (tenant_key, variant_key)
    WHERE status = 'draft';
```
...
CREATE TABLE environment_activations (
    environment_key VARCHAR(30) NOT NULL REFERENCES environments(id) ON DELETE CASCADE,
    variant_key VARCHAR(50) NOT NULL REFERENCES template_variants(id) ON DELETE CASCADE,
    version_key INTEGER NOT NULL,
    ...
    PRIMARY KEY (environment_key, variant_key),
    FOREIGN KEY (variant_key, version_key) REFERENCES template_versions(variant_key, id) ON DELETE CASCADE
);
...
CREATE TABLE environment_activations (
    ...
    PRIMARY KEY (tenant_key, environment_key, variant_key),
...
CONSTRAINT fk_requests_variant_version
    FOREIGN KEY (tenant_key, variant_key, version_key)
    REFERENCES template_versions(tenant_key, variant_key, id)
    ON DELETE CASCADE
```
...
`VariantSummary`, `TemplateVersion`, `VersionSummary` — **no changes needed**. These are always queried in a context where `tenantId` is already known. The DB has `tenant_key` for FK integrity, but the application doesn't need to carry it in these models.

---

## Step 3: Query Simplification

Many queries currently JOIN to `document_templates` solely for tenant isolation. With `tenant_key` directly on `template_variants`, these JOINs can be removed.

### Pattern: Before → After

```sql
-- BEFORE (JOIN for tenant check)
SELECT tv.id, tv.template_key, tv.attributes, tv.created_at, tv.last_modified
FROM template_variants tv
JOIN document_templates dt ON tv.template_key = dt.id
WHERE tv.template_key = :templateId
  AND dt.tenant_key = :tenantId

-- AFTER (direct tenant check)
SELECT tv.id, tv.tenant_key, tv.template_key, tv.attributes, tv.created_at, tv.last_modified
FROM template_variants tv
WHERE tv.template_key = :templateId
