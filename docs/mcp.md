...
| `list_component_types` | Describe every editor component type — slots, `allowedChildren`, applicable style keys, inspector fields, default props/styles. The single source of truth is the TypeScript registry (`createDefaultRegistry()` in `modules/editor`); a JSON snapshot is generated at editor build time and read at runtime. Use this to design templates from scratch or validate that referenced node types exist. |
| `get_component_type`   | Fetch a single component type's descriptor by `type`.                                                                                                                                                                                                                                                                                                                                                 |
