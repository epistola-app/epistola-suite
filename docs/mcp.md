...
| `analyze_template_data`   | Check data against the contract of the version `preview_document` would render, without rendering. Returns the missing fields (required ones always, optional ones only when the template reads them), the invalid fields, and `missingDataSchema`, a JSON Schema of just the missing part, so an agent can ask for exactly the data the template still needs. See [Missing and invalid data](data-contracts.md#missing-and-invalid-data).                                                                                   |
