...
- [ ] 1.1 Create `css/design-system/` directory structure
- [ ] 1.2 Create `tokens.css` with CSS custom properties (colors, spacing, radius)
- [ ] 1.3 Create `index.css` that imports all component CSS files
- [ ] 1.4 Update `main.css` to import design-system

## 2. Button Component (PoC Complete)
...
- [ ] 2.6 Move button styles from `main.css` to `design-system/button.css`

## 3. Input Component

- [ ] 3.1 Define `.input` base styles (border, padding, focus ring)
- [ ] 3.2 Define `.textarea` styles
- [ ] 3.3 Add error state (`.input-error` and `aria-invalid`)
- [ ] 3.4 Add disabled state styles
- [ ] 3.5 Add size variants if needed (`.input-sm`, `.input-lg`)

## 4. Badge Component

- [ ] 4.1 Define `.badge` base styles (inline-flex, pill shape)
- [ ] 4.2 Add variant classes (`.badge-primary`, `.badge-secondary`, `.badge-destructive`, `.badge-outline`)
- [ ] 4.3 Migrate existing `.badge-draft`, `.badge-published`, `.badge-archived` to new system

## 5. Card Component

- [ ] 5.1 Define `.card` base styles (background, border/shadow, radius)
- [ ] 5.2 Define `.card-header`, `.card-content`, `.card-footer` sections
- [ ] 5.3 Apply to `.form-section` in Thymeleaf templates

## 6. Update Thymeleaf Templates

- [ ] 6.1 Update `tenants/list.html` to use new component classes
- [ ] 6.2 Update `templates/list.html` to use new component classes
- [ ] 6.3 Update `templates/detail.html` to use new component classes
- [ ] 6.4 Remove button showcase section from `detail.html`
...
- [ ] 8.2 Add inline documentation comments to CSS files
