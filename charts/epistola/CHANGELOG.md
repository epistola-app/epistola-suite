...
- **`database.cnpg.managedRoles` / `postInitApplicationSQL` / `postInitSQL` — configure extra CNPG roles and bootstrap SQL.** Passed through to the CNPG `Cluster` (`spec.managed.roles` and `spec.bootstrap.initdb.postInit*SQL`), so a chart-managed CNPG cluster can provision the separate migration role and its grants for the two-role split above. See the CNPG two-role recipe in `docs/deployment.md` (validate the privilege SQL against your CNPG/Postgres version).
