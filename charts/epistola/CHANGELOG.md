...
- **`support.telemetry.enabled` — forward logs + metrics to epistola-hub.** Off by default. When enabled (and `support.enabled=true`), the app ships application logs and metrics to the hub over OTLP-over-gRPC; rendered as `EPISTOLA_SUPPORT_TELEMETRY_ENABLED` on the app Deployment. It also requires the installation's `support-telemetry` entitlement from the hub before anything is shipped (the hub gates the stream). The OTLP endpoint is the hub's own gRPC endpoint, derived at runtime by the support module — nothing to configure. See [`docs/adr/0006-shipping-logs-and-metrics-to-hub.md`](../../docs/adr/0006-shipping-logs-and-metrics-to-hub.md).
