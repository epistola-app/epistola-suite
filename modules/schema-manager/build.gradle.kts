...
val copyDistToResources by tasks.registering(Copy::class) {
    ...
    from("dist")
    into(layout.buildDirectory.dir("resources/main/META-INF/resources/schema-manager"))

    // Ensure task re-runs if output directory is missing
    outputs.upToDateWhen { layout.buildDirectory.dir("resources/main/META-INF/resources/schema-manager").get().asFile.exists() }
}
