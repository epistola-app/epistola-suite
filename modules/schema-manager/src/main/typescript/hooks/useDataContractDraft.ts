...
export interface DataContractStore {
  schema: JsonSchema | null;
  dataExamples: DataExample[];
 ...
 */
export function useDataContractDraft(
  store: DataContractStore,
  callbacks: SaveCallbacks,
): DataContractDraft {
  // Get initial values from store
  const storeSchema = store.schema;
  const storeExamples = store.dataExamples;
  const setStoreSchema = store.setSchema;
  const setStoreExamples = store.setDataExamples;
