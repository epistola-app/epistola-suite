...
class ClusterScheduledTaskScheduleCalculator {
     ...
     */
    fun initialDueAt(definition: ClusterScheduledTaskDefinition, now: OffsetDateTime = EpistolaClock.offsetDateTime()): OffsetDateTime = when (val schedule = definition.schedule) {
     ...
     */
    fun nextAfterSuccess(task: ClusterScheduledTask, now: OffsetDateTime = EpistolaClock.offsetDateTime()): OffsetDateTime = when (task.scheduleKind) {
     ...
     * occurrence.
     */
    fun nextAfterFailure(task: ClusterScheduledTask, now: OffsetDateTime = EpistolaClock.offsetDateTime(), retryDelayMs: Long, maxRetryDelayMs: Long): OffsetDateTime = when (task.failurePolicy) {
