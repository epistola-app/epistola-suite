 ...
 *
 * Throws if the version doesn't exist, is archived, or the environment doesn't belong to the tenant.
 */
...
class PublishToEnvironmentHandler(
    ...
    override fun handle(command: PublishToEnvironment): PublishToEnvironmentResult = jdbi.inTransaction<PublishToEnvironmentResult, Exception> { handle ->
        ...
        // 2. Verify version exists and is not archived
        ...
        if (versionStatus == "archived") {
            throw VersionArchivedException(command.versionId.tenantKey, command.versionId.key)
        }

        // 3. Publish the version (handles contract auto-publish, theme snapshot, etc.)
