...
class ListDocumentTemplatesHandler(
    ...
    override fun handle(query: ListDocumentTemplates): List<DocumentTemplate> = jdbi.withHandle<List<DocumentTemplate>, Exception> { handle ->
        val sql = buildString {
            append("SELECT id, tenant_id, name, theme_id, schema, data_model, data_examples, pdfa_enabled, created_at, last_modified FROM document_templates WHERE tenant_id = :tenantId")
