...
CREATE DOMAIN ATTRIBUTE_ID AS VARCHAR(50)
    CHECK (VALUE ~ '^[a-z][a-z0-9]*(-[a-z0-9]+)*$');
...
-- Composite PK (tenant_id, template_id, id) ensures variants are unique per template, not per tenant
...
-- Composite PK (tenant_id, template_id, variant_id, id) matches template_variants hierarchy
CREATE TABLE template_versions (
    ...
    created_by UUID REFERENCES users(id),
    PRIMARY KEY (tenant_id, template_id, variant_id, id),
    FOREIGN KEY (tenant_id, template_id, variant_id) REFERENCES template_variants(tenant_id, template_id, id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_one_draft_per_variant
    ON template_versions (tenant_id, template_id, variant_id)
    WHERE status = 'draft';
...
COMMENT ON COLUMN template_versions.template_id IS 'Parent template (part of composite key with tenant_id and variant_id)';
...
-- Composite PK includes template_id to match template_variants and template_versions hierarchy
CREATE TABLE environment_activations (
    ...
    PRIMARY KEY (tenant_id, environment_id, template_id, variant_id),
    FOREIGN KEY (tenant_id, environment_id) REFERENCES environments(tenant_id, id) ON DELETE CASCADE,
    FOREIGN KEY (tenant_id, template_id, variant_id) REFERENCES template_variants(tenant_id, template_id, id) ON DELETE CASCADE,
    FOREIGN KEY (tenant_id, template_id, variant_id, version_id) REFERENCES template_versions(tenant_id, template_id, variant_id, id) ON DELETE CASCADE
);
...
COMMENT ON COLUMN environment_activations.template_id IS 'Parent template (part of composite key)';
...
-- Composite PK (tenant_id, id) ensures attribute names are unique per tenant
CREATE TABLE variant_attribute_definitions (
    id ATTRIBUTE_ID NOT NULL,
