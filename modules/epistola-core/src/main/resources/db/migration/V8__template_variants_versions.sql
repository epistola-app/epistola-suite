...
-- All tenant-owned entities use composite PKs (tenant_key, id) for tenant isolation
...
-- Composite PK (tenant_key, template_key, id) ensures variants are unique per template, not per tenant
CREATE TABLE template_variants (
    ...
    PRIMARY KEY (tenant_key, template_key, id),
...
-- Composite PK (tenant_key, template_key, variant_key, id) matches template_variants hierarchy
CREATE TABLE template_versions (
    ...
    created_by UUID REFERENCES users(id),
    PRIMARY KEY (tenant_key, template_key, variant_key, id),
...
CREATE UNIQUE INDEX idx_one_draft_per_variant
    ON template_versions (tenant_key, template_key, variant_key)
    WHERE status = 'draft';
...
COMMENT ON COLUMN template_versions.template_key IS 'Parent template (part of composite key with tenant_key and variant_key)';
...
-- Composite PK includes template_key to match template_variants and template_versions hierarchy
CREATE TABLE environment_activations (
    ...
    PRIMARY KEY (tenant_key, environment_key, template_key, variant_key),
...
-- Composite PK (tenant_key, id) ensures attribute names are unique per tenant
