...
--   - Composite PKs for tenant isolation
...
COMMENT ON COLUMN stencils.id IS 'URL-safe slug identifier unique within the tenant (e.g., corporate-header)';
...
CREATE TABLE stencil_versions (
    ...
    created_by UUID REFERENCES users(id),
    PRIMARY KEY (tenant_key, stencil_key, id),
    FOREIGN KEY (tenant_key, stencil_key) REFERENCES stencils(tenant_key, id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_one_draft_per_stencil
    ON stencil_versions (tenant_key, stencil_key)
    WHERE status = 'draft';
