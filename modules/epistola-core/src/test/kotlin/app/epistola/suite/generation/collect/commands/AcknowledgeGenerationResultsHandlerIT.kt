...
class AcknowledgeGenerationResultsHandlerIT : IntegrationTestBase() {
    ...
    @Autowired
    private lateinit var cursors: ConsumerPartitionCursorRepository
    ...
    fun `advances cursors for every assigned partition to acknowledgeUpTo`() {
        ...
        val read = cursors.cursorsFor(t, c, setOf(0, 7, 33, 1))
    ...
    fun `late ack cannot move a cursor backwards`() {
        ...
        assertThat(cursors.cursorsFor(t, c, setOf(0))[0]).isEqualTo(1000L)
    }
    ...
    fun `empty partition set is a no-op`() {
        ...
        assertThat(cursors.cursorsFor(t, c, setOf(0))[0]).isEqualTo(0L)
    }
    ...
    fun `acknowledgeUpTo of zero is a no-op`() {
        ...
        val read = cursors.cursorsFor(t, c, setOf(0, 1, 2))
    ...
    fun `cursors are isolated per (tenant, consumer)`() {
        ...
        assertThat(cursors.cursorsFor(tA, cA, setOf(0))[0]).isEqualTo(100L)
        assertThat(cursors.cursorsFor(tB, cA, setOf(0))[0]).isEqualTo(200L)
        assertThat(cursors.cursorsFor(tA, cB, setOf(0))[0]).isEqualTo(300L)
    }
}
