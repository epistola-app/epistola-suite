...
/**
 * The global default for a feature with no tenant override: support-tier features follow
...
class FeatureToggleDefaultsTest {
    ...
    @Test
    fun `support-tier features default to the support tier`() {
        val on = service(supportEnabled = true)
        val off = service(supportEnabled = false)
        for (feature in KnownFeatures.SUPPORT_TIER) {
