...
class ThemeIdTest {
    ...
    inner class InvalidSlugs {
        ...
        fun `should reject slug starting with number`() {
            assertThrows<IllegalArgumentException> { ThemeKey.of("123abc") }
        }

        @Test
        fun `should reject slug starting with hyphen`() {
            assertThrows<IllegalArgumentException> { ThemeKey.of("-abc") }
        }
        ...
        fun `should reject slug with consecutive hyphens`() {
            assertThrows<IllegalArgumentException> { ThemeKey.of("abc--def") }
        }

        @Test
        fun `should reject slug with uppercase letters`() {
            assertThrows<IllegalArgumentException> { ThemeKey.of("MyTheme") }
        }

        @Test
        fun `should reject slug with spaces`() {
            assertThrows<IllegalArgumentException> { ThemeKey.of("my theme") }
        }

        @Test
        fun `should reject slug with underscores`() {
            assertThrows<IllegalArgumentException> { ThemeKey.of("my_theme") }
        }
        ...
        fun `should reject empty slug`() {
            assertThrows<IllegalArgumentException> { ThemeKey.of("") }
        }
