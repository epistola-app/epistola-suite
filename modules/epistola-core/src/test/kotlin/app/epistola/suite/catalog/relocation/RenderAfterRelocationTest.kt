...
/**
 * A published document must keep rendering after anything it depends on moves. Each test publishes
 * a template, relocates what it uses, and renders the published version for real. The resolution
 * queries alone (covered in [RuntimeResolutionAfterRelocationTest]) do not exercise what the renderer
 * runs first, notably the font integrity check against the fingerprints pinned at publish.
 */
class RenderAfterRelocationTest : RelocationTestSupport() {
     ...
     */
    @Test
    fun `a published document still renders after its theme's font family moves`() {
    ...
    @Test
    fun `a published document still renders after its theme's font family is renamed`() {
    ...
    @Test
    fun `a published document still renders after the image it shows moves`() {
