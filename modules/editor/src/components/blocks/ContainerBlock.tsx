...
export function ContainerBlockComponent({ block, isSelected = false }: ContainerBlockProps) {
  ...
  return (
    <div className={`rounded-lg border ${isSelected ? 'bg-gray-50 border-dashed border-gray-300' : 'border-transparent'}`}>
      {isSelected && (
        <div className="text-xs text-gray-400 px-3 pt-2 font-medium">CONTAINER</div>
      )}
      <div
        ref={setNodeRef}
        style={block.styles}
      ...
      >
        {block.children.length === 0 ? (
          <span className="text-gray-400 text-sm">{isSelected ? 'Drop blocks here' : 'Empty container'}</span>
        ) : (
