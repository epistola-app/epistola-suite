...
export class ThemeEditorState extends EventTarget {
  ...
  computePatchPayload(): Record<string, unknown> {
    ...
    if (
      JSON.stringify(this._current.documentStyles) !== JSON.stringify(this._original.documentStyles)
    ) {
      patch.documentStyles = this._current.documentStyles;
    }

    if (
      JSON.stringify(this._current.pageSettings) !== JSON.stringify(this._original.pageSettings)
    ) {
      if (this._current.pageSettings) {
    ...
    if (
      JSON.stringify(this._current.blockStylePresets) !==
      JSON.stringify(this._original.blockStylePresets)
    ) {
