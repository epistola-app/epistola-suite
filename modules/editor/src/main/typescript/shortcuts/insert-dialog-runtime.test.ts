...
async function runCommand(
  commandId: string,
  context: InsertDialogShortcutRuntimeContext,
) {
...
describe('insert dialog shortcut runtime', () => {
  it('defines insertDialog-context keybindings including quick-select', () => {
    ...
    expect(registry.keybindings.some((binding) => binding.commandId === INSERT_DIALOG_SHORTCUT_COMMAND_IDS.closeOrBack)).toBe(true)
    expect(registry.keybindings.some((binding) => binding.commandId === 'insertDialog.quick-select.key-1')).toBe(true)
  })
