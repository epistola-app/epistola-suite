...
describe('text shortcut runtime', () => {
  it('defines text-context keybindings for mark and line-break commands', () => {
    ...
    const lineBreakBindings = getTextShortcutBindingsForCommandId(TEXT_SHORTCUT_COMMAND_IDS.lineBreakShiftEnter)
  ...
  it('builds bubble titles from binding display values', () => {
    ...
    expect(getTextBubbleTitle(TEXT_SHORTCUT_COMMAND_IDS.underline, 'Underline')).toBe('Underline (Ctrl/Cmd + U)')
  })
})
