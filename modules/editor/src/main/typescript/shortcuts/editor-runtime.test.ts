...
describe('editor shortcut runtime registry', () => {
  ...
  it('uses editor context for core shortcuts and global context for leader chords', () => {
    ...
    const saveBinding = registry.keybindings.find((binding) => binding.commandId === EDITOR_SHORTCUT_COMMAND_IDS.save)
    expect(saveBinding).toBeDefined()
    expect(saveBinding?.context).toBe('editor')
    expect(saveBinding?.keys).toContain('mod+s')

    const previewBinding = registry.keybindings.find((binding) => binding.commandId === EDITOR_SHORTCUT_COMMAND_IDS.togglePreview)
  ...
  it('preserves leader alias keys for opening shortcuts help', () => {
    const registry = getEditorShortcutRegistry()
    const helpBinding = registry.keybindings.find((binding) => binding.commandId === EDITOR_SHORTCUT_COMMAND_IDS.openShortcutsHelp)
