...
const GROUP_DEFINITIONS: readonly GroupDefinition[] = [
  ...
  },
  {
    ...
    fullWidth: false,
    projectItems: () => buildCommandRowsFromRegistry(getEditorShortcutRegistry(), { category: 'Leader', idPrefix: 'leader' }),
  },
  {
    id: 'core',
    title: 'Core',
    fullWidth: false,
    projectItems: () => buildCommandRowsFromRegistry(getEditorShortcutRegistry(), { category: 'Core', idPrefix: 'core' }),
  },
  {
    id: 'text',
    title: 'Text',
    fullWidth: false,
    projectItems: () => buildCommandRowsFromRegistry(getTextShortcutRegistry(), { category: 'Text', idPrefix: 'text' }),
  },
  {
    id: 'insert',
    title: 'Insert',
    fullWidth: false,
    projectItems: () => buildCommandRowsFromRegistry(getInsertDialogShortcutRegistry(), { category: 'Insert', idPrefix: 'insert' }),
  },
  {
    id: 'resize',
    title: 'Resize',
    fullWidth: false,
    projectItems: () => buildCommandRowsFromRegistry(getResizeShortcutRegistry(), { category: 'Resize', idPrefix: 'resize' }),
  },
]
...
function toProjectionItem(seed: ShortcutRowSeed, activeStrokes: Set<string>): ShortcutHelperProjectionItem {
  const isActive = seed.matchStrokes.some((stroke) => activeStrokes.has(normalizeStrokeToken(stroke)))
  return {
    id: seed.id,
...
export function buildShortcutHelperProjection(
  ...
  const activeStrokes = new Set((options.activeStrokes ?? []).map((stroke) => normalizeStrokeToken(stroke)))

  const groups: ShortcutHelperProjectionGroup[] = []
  for (const groupDefinition of GROUP_DEFINITIONS) {
    const visibleItems = groupDefinition.projectItems()
...
export function collectHelperProjectionDisplays(group: ShortcutHelperProjectionGroup): readonly string[] {
  return group.items.map((item) => item.keys)
}

export function collectHelperProjectionBindings(group: ShortcutHelperProjectionGroup): readonly string[] {
