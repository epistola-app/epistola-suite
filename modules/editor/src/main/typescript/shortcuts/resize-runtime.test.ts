...
describe('resize shortcut runtime', () => {
  it('defines resize-handle keybindings', () => {
    ...
    expect(registry.keybindings.find((binding) => binding.commandId === RESIZE_SHORTCUT_COMMAND_IDS.growPreviewWidth)?.keys).toEqual(['ArrowLeft'])
    expect(registry.keybindings.find((binding) => binding.commandId === RESIZE_SHORTCUT_COMMAND_IDS.shrinkPreviewWidth)?.keys).toEqual(['ArrowRight'])
  })
