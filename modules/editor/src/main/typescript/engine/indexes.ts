...
export function getAncestorPath(
  ...
  if (path.length > 0 && path[0] !== doc.root) {
    throw new Error(`Node ${nodeId} is not connected to document root ${doc.root}`);
  }
 ...
 */
export function isAncestor(nodeId: NodeId, ancestorId: NodeId, indexes: DocumentIndexes): boolean {
  const visited = new Set<NodeId>();
