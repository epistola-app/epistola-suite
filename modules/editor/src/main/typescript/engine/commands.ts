...
function applyUpdateNodeProps(doc: TemplateDocument, cmd: UpdateNodeProps): CommandResult {
  const node = doc.nodes[cmd.nodeId];
  if (!node) return err(`Node ${cmd.nodeId} not found`);
...
function applyUpdateNodeStyles(doc: TemplateDocument, cmd: UpdateNodeStyles): CommandResult {
  const node = doc.nodes[cmd.nodeId];
  if (!node) return err(`Node ${cmd.nodeId} not found`);
...
function applySetStylePreset(doc: TemplateDocument, cmd: SetStylePreset): CommandResult {
  const node = doc.nodes[cmd.nodeId];
  if (!node) return err(`Node ${cmd.nodeId} not found`);
