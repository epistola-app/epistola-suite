...
export interface InspectorField {
  ...
  /**
   * Optional predicate gating whether this field is shown in the inspector,
   * evaluated against the node's current props. Use it for fields that only
   * apply to a subset of a component's configuration (e.g. a bullet-style
   * picker that is only relevant when `listType === 'bullet'`). The predicate
   * is editor-only — it is a function, so it is not serialized into
   * `component-registry.json` and never reaches backend consumers (MCP).
   */
...
export interface ComponentDefinition {
   ...
   * `apps/epistola-demo/src/main/resources/epistola/catalogs/demo/`.
   */
...
export function createDefaultRegistry(): ComponentRegistry {
  ...
  registry.register({
    ...
    inspector: [
      ...
      {
        // Bullet glyph — mirrors the Text component's `bullet_list` `listStyle`
        // so a datalist bullet list and a typed bullet list render the same marker.
        // Only relevant when listType is "bullet".
        key: 'bulletStyle',
        label: 'Bullet Style',
        type: 'select',
        options: [
        ...
        visibleWhen: (props) => (props.listType ?? 'bullet') === 'bullet',
      },
    ],
    defaultProps: {
      expression: { raw: '', language: 'jsonata' },
    ...
    examples: [
      ...
      {
        name: 'styled-bullet-list',
        description:
          "A bulleted list with a non-default marker glyph via bulletStyle. Mirrors the Text component's bullet styles (disc, circle, square, dash); the same glyph renders whether the list is typed in a Text block or generated by a datalist. Note: circle/square only render with an embedding font — disc and dash are safe in standard PDFs.",
        fragment: {
          rootNodeId: 'n-datalist-styled',
          nodes: {
            'n-datalist-styled': {
              id: 'n-datalist-styled',
              type: 'datalist',
              slots: ['s-datalist-styled-template'],
              props: {
                expression: { raw: 'tags', language: 'jsonata' },
                itemAlias: 'tag',
                listType: 'bullet',
                bulletStyle: 'dash',
              },
              ...
              props: {
                content: {
                  ...
                  content: [
                    {
                      type: 'paragraph',
                      content: [{ type: 'expression', attrs: { expression: 'tag' } }],
                    },
                  ],
                },
              },
          ...
          slots: {
            's-datalist-styled-template': {
              id: 's-datalist-styled-template',
              nodeId: 'n-datalist-styled',
              name: 'item-template',
              children: ['n-datalist-styled-item'],
            },
          },
        },
      },
