  ...
  return [
    ...new Set(
      bindings.flatMap((binding) =>
        binding.keys.flatMap((key) => toProseMirrorKeysFromShortcutStroke(key)),
      ),
    ),
  ];
}
...
function markKeymap(schema: Schema) {
  ...
  if (schema.marks.strong) {
    assignTextCommandBindings(
      bindings,
      TEXT_SHORTCUT_COMMAND_IDS.bold,
  ...
  if (schema.marks.em) {
    assignTextCommandBindings(
      bindings,
      TEXT_SHORTCUT_COMMAND_IDS.italic,
  ...
  if (schema.marks.underline) {
    assignTextCommandBindings(
      bindings,
      TEXT_SHORTCUT_COMMAND_IDS.underline,
