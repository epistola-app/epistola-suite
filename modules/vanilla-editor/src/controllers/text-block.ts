...
export class TextBlockController extends Controller {
  ...
  connect(): void {
    ...
    this.onDocumentClickBound = (event: Event) => this.handleDocumentClick(event);
  ...
  private openChipPopoverAtPos(pos: number): void {
    ...
    if (!this.chipPopover || !this.chipInput || !this.chipPreview || !this.chipDropdown) {
      return;
    }
    ...
    this.chipInput.setSelectionRange(this.chipInput.value.length, this.chipInput.value.length);
  ...
  private createChipPopover(): void {
    ...
    input.type = 'text';
    input.className = 'form-control form-control-sm expression-chip-popover-input';
  ...
  private handleChipInputKeydown(event: KeyboardEvent): void {
    ...
    const items = this.chipDropdown.querySelectorAll<HTMLElement>('.expression-suggestion-item');
    const active = this.chipDropdown.querySelector<HTMLElement>('.expression-suggestion-item.active');
    ...
    if (event.key === 'ArrowDown') {
      event.preventDefault();
      this.highlightChipItem(items[activeIndex < items.length - 1 ? activeIndex + 1 : 0]);
      return;
    }

    if (event.key === 'ArrowUp') {
      event.preventDefault();
      this.highlightChipItem(items[activeIndex > 0 ? activeIndex - 1 : items.length - 1]);
      return;
    }
  ...
  private updateChipDropdown(): void {
    ...
    const cursorPos = this.chipInput.selectionStart ?? this.chipInput.value.length;
  ...
  private selectChipSuggestion(value: string): void {
    ...
    const cursorStart = this.chipInput.selectionStart ?? this.chipInput.value.length;
    const cursorEnd = this.chipInput.selectionEnd ?? this.chipInput.value.length;
  ...
  private async updateChipPreview(): Promise<void> {
    ...
    if (!result.success) {
      this.renderChipPreview(result.error ?? 'Expression evaluation failed', true);
      return;
    }
  ...
  private closeChipPopover(cancel: boolean): void {
    ...
    const shouldDeleteOnCancel = cancel && this.activeChipIsNew && this.activeChipPos !== null;
