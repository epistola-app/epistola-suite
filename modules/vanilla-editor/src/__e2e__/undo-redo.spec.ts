...
test.describe("Undo / Redo - Basic Operations", () => {
  ...
  test("multiple undos walk back through history", async ({
    page,
    appHelpers,
  }) => {
    const initialCount = await appHelpers.getBlockCount();
...
test.describe("Undo / Redo - Redo Operations", () => {
  test("redo after undo restores the block", async ({ page, appHelpers }) => {
    ...
    await expect(
      appHelpers.getBlockByType(BLOCK_TYPES.text).last(),
    ).toBeVisible();
  });

  test("redo delete after undo delete removes block again", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addBlock(BLOCK_TYPES.text);
  ...
  test("multiple redos walk forward through history", async ({
    page,
    appHelpers,
  }) => {
    const initialCount = await appHelpers.getBlockCount();
...
test.describe("Undo / Redo - History Management", () => {
  test("new action after undo clears redo history", async ({
    page,
    appHelpers,
  }) => {
    const initialCount = await appHelpers.getBlockCount();
...
test.describe("Undo / Redo - Complex Scenarios", () => {
  test("history survives complex add-delete-undo-redo sequence", async ({
    page,
    appHelpers,
  }) => {
    const initialCount = await appHelpers.getBlockCount();
