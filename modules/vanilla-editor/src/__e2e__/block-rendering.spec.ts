...
test.describe("Document Styles - UI", () => {
  test("document styles modal roundtrip preserves values and units", async ({
    ...
    await expect(page.locator("#ve-doc-font-weight")).toHaveValue("600");
    await expect(page.locator("#ve-doc-color")).toHaveValue("#123456");
    await expect(page.locator("#ve-doc-line-height")).toHaveValue("1.7");
    await expect(page.locator("#ve-doc-letter-spacing-value")).toHaveValue(
      "0.5",
    );
    await expect(page.locator("#ve-doc-letter-spacing-unit")).toHaveValue("em");
    await expect(page.locator("#ve-doc-text-align")).toHaveValue("right");
    await expect(page.locator("#ve-doc-bg-color")).toHaveValue("#f0f0f0");
  });

  test("document styles modal applies font size and color to newly added text block", async ({
    page,
    appHelpers,
  }) => {
    await page.click('[data-editor-app-action="open-document-styles"]');
  ...
  test("document styles modal preserves font and letter spacing units across save/reopen", async ({
    ...
    await expect(page.locator("#ve-doc-letter-spacing-unit")).toHaveValue("px");

    await page.fill("#ve-doc-font-size-value", "11");
  ...
  test("document styles modal applies font weight text align and letter spacing to text block", async ({
    page,
    appHelpers,
  }) => {
    await page.click('[data-editor-app-action="open-document-styles"]');
...
test.describe("Block Styles - UI", () => {
  test("block styles modal roundtrip preserves values for selected block", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addTextBlock();
  ...
  test("block styles override document styles for selected block", async ({
    page,
    appHelpers,
  }) => {
    await page.click('[data-editor-app-action="open-document-styles"]');
  ...
  test("clear block styles removes overrides and falls back to document styles", async ({
    page,
    appHelpers,
  }) => {
    await page.click('[data-editor-app-action="open-document-styles"]');
...
test.describe("Block Rendering - Text Blocks", () => {
  test("text block renders with ProseMirror editor", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addTextBlock();
  ...
  test.skip("text block inside container renders correctly", async ({
    ...
    const nestedText = container.locator(
...
test.describe("Block Rendering - Container Blocks", () => {
  test("empty container shows placeholder", async ({ page, appHelpers }) => {
    ...
    await expect(block.locator(SELECTORS.emptyState)).toContainText(
      "Drop blocks here",
    );
  });
  ...
  test.skip("container with children renders all nested blocks", async ({
    ...
    const textBlocks = container.locator(
      `[data-block-type="${BLOCK_TYPES.text}"]`,
    );
...
test.describe("Block Rendering - Conditional Blocks", () => {
  test("conditional block renders expression input", async ({
    page,
    appHelpers,
  }) => {
  ...
  test("conditional block empty children shows placeholder", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addConditionalBlock();
  ...
  test.skip("conditional block with children renders nested content", async ({
    ...
    const nestedText = conditional.locator(
      `[data-block-type="${BLOCK_TYPES.text}"]`,
    );
    await expect(nestedText).toBeVisible();
...
test.describe("Block Rendering - Loop Blocks", () => {
  test("loop block renders expression input and alias fields", async ({
    page,
    appHelpers,
  }) => {
  ...
  test("loop block empty children shows placeholder", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addLoopBlock();
...
test.describe("Block Rendering - Columns Blocks", () => {
  test("columns block renders correct number of columns", async ({
    page,
    appHelpers,
  }) => {
  ...
  test("empty columns show placeholder in each column", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addColumnsBlock();
    ...
    const placeholders = block.locator(
      `${SELECTORS.columnWrapper} ${SELECTORS.emptyState}`,
    );
...
test.describe("Block Rendering - Table Blocks", () => {
  test("table block renders correct row and column counts", async ({
    page,
    appHelpers,
  }) => {
    ...
    await expect(block.locator("tr:first-child th[data-cell-id]")).toHaveCount(
      3,
    );
  });

  test("empty table cells show add text placeholder", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addTableBlock();
...
test.describe("Block Rendering - Page Blocks", () => {
  test("page header renders info banner and empty placeholder", async ({
    page,
    appHelpers,
  }) => {
    ...
    await expect(block).toHaveAttribute(
      "data-block-type",
      BLOCK_TYPES.pageheader,
    );
    await expect(block.locator(".alert-info")).toContainText(
      "top of every page",
    );
    await expect(block.locator(SELECTORS.emptyState)).toContainText(
      "Add header content",
  ...
  test("page footer renders info banner and empty placeholder", async ({
    page,
    appHelpers,
  }) => {
  ...
  test("pagebreak renders dashed separator with label", async ({
    page,
    appHelpers,
  }) => {
...
test.describe("Block Rendering - Block Headers", () => {
  test("each block has header with drag handle, badge, and delete button", async ({
    page,
    appHelpers,
  }) => {
    await appHelpers.addTextBlock();
    ...
    const containerBlock = appHelpers
      .getBlockByType(BLOCK_TYPES.container)
      .last();
  ...
  test("badge text matches block type", async ({ page, appHelpers }) => {
    ...
    await expect(
      appHelpers
        .getBlockByType(BLOCK_TYPES.text)
        .last()
        .locator(`${SELECTORS.blockHeader} .badge`),
    ...
    await expect(
      appHelpers
        .getBlockByType(BLOCK_TYPES.loop)
        .last()
        .locator(`${SELECTORS.blockHeader} .badge`),
