...
-- Mark documents generated by load tests for cleanup
ALTER TABLE documents ADD COLUMN is_load_test BOOLEAN NOT NULL DEFAULT false;
CREATE INDEX idx_documents_load_test ON documents(is_load_test) WHERE is_load_test = true;
...
COMMENT ON COLUMN load_test_requests.document_id IS 'Reference to generated document. Document is deleted immediately after test completes.';

COMMENT ON COLUMN documents.is_load_test IS 'Flag for load test documents. These are deleted immediately after test completes.';
