...
class RegisterCatalogHandler(
    ...
    override fun handle(command: RegisterCatalog): Catalog {
        ...
        return jdbi.inTransaction<Catalog, Exception> { handle ->
            handle.createUpdate(
                """
                INSERT INTO catalogs (id, tenant_key, name, description, type, mutability, source_url, source_auth_type, source_auth_credential, installed_release_version, created_at, last_modified)
                VALUES (:id, :tenantKey, :name, :description, 'SUBSCRIBED', 'READ_ONLY', :sourceUrl, :authType, :authCredential, :releaseVersion, NOW(), NOW())
