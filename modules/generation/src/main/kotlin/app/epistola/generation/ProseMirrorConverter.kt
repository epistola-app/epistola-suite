...
class ProseMirrorConverter(
    ...
    private fun convertHeading(
        ...
        val paragraphs = renderHardBreakSegments(content, data, loopContext, fontCache, face) { paragraph, index, lastIndex ->
     ...
     * font / margin / style / accessibility policy (the only thing paragraphs
     * and headings actually differ on); padding, spacing-ratio and the
     * empty-segment placeholder are handled centrally so the two node types
     * can't drift.
     */
