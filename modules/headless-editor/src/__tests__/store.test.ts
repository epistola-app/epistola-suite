...
describe('BlockTree', () => {
  ...
  describe('findParent', () => {
    ...
    it('should find parent of block inside a column', () => {
      const columnsBlock: Block = {
        ...
        type: 'columns',
        columns: [
          { id: 'col-1', size: 1, children: [{ id: 'child-in-col', type: 'text', content: 'x' }] },
        ],
      };
    ...
    it('should find parent of block inside a table cell', () => {
      const tableBlock: Block = {
        ...
        rows: [
          {
            id: 'row-1',
            cells: [{ id: 'cell-1', children: [{ id: 'child-in-cell', type: 'text', content: 'x' }] }],
          },
        ],
      };
  ...
  describe('findColumn', () => {
    ...
    it('should return null for non-existent column', () => {
      const blocks: Block[] = [
        { id: 'columns-1', type: 'columns', columns: [{ id: 'col-a', size: 1, children: [] }] },
      ];
      ...
      const blocks: Block[] = [
        {
          ...
          type: 'table',
          rows: [
            { id: 'row-1', cells: [{ id: 'cell-x', children: [] }] },
          ],
        },
      ];
      ...
      const blocks: Block[] = [
        {
          ...
          rows: [
            {
              id: 'row-1',
              cells: [{ id: 'cell-1', children: [{ id: 'in-cell', type: 'text', content: 'Remove' }] }],
            },
          ],
        },
      ];
      ...
      const blocks: Block[] = [
        {
          ...
          type: 'columns',
          columns: [
            { id: 'col-1', size: 1, children: [{ id: 'in-col', type: 'text', content: 'Before' }] },
          ],
        },
      ];

      const result = BlockTree.updateBlock(blocks, 'in-col', { content: 'After' });

      const columnsBlock = result[0] as { columns: { children: Block[] }[] };
      expect((columnsBlock.columns[0].children[0] as { content: string }).content).toBe('After');
    });
      ...
      const blocks: Block[] = [
        {
          ...
          rows: [
            {
              id: 'row-1',
              cells: [{ id: 'cell-1', children: [{ id: 'in-cell', type: 'text', content: 'Before' }] }],
            },
          ],
        },
      ];

      const result = BlockTree.updateBlock(blocks, 'in-cell', { content: 'After' });

      const tableBlock = result[0] as { rows: { cells: { children: Block[] }[] }[] };
      expect((tableBlock.rows[0].cells[0].children[0] as { content: string }).content).toBe('After');
    });
  ...
  describe('moveBlock', () => {
    ...
    it('should return original blocks when block ID not found', () => {
      const blocks: Block[] = [{ id: 'block-1', type: 'text', content: 'Only' }];
  ...
  describe('getChildren', () => {
    ...
    it('should return column children', () => {
      ...
      const blocks: Block[] = [
        { id: 'columns-1', type: 'columns', columns: [{ id: 'col-1', size: 1, children: [child] }] },
      ];
    ...
    it('should return cell children', () => {
      const child: Block = { id: 'cell-child', type: 'text', content: 'In cell' };
      const blocks: Block[] = [
        {
          id: 'table-1',
          type: 'table',
