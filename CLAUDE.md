...
17. **Vulnerability records are repository-owned** - Add disclosed vulnerabilities as dated Markdown files under `vulnerabilities/` with JSON frontmatter and a full Markdown description. Run `pnpm vulnerabilities:render` after every record change and `pnpm vulnerabilities:check` before committing. GitHub Security Advisories are synchronized publication mirrors; the repository record remains canonical. Do not publish a mirror before a patched release is recorded.
