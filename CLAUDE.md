...
- **Extension points** use small SPIs in core, introduced one-at-a-time as features need them. Pattern: feature module ships a `TemplateDetailTab` `@Component` + a UI handler returning an HTMX fragment; host page collects all contributions and renders a tab strip with `hx-get` lazy-load. No SPI exists yet — add the first one when the first contributing feature needs it.
