...
16. **Follow inline style hygiene rules** - All inline `style="..."`, `classList.*`, and `var(--ep-*, <fallback>)` are enforced by [`InlineStyleHygieneTest`](docs/style-hygiene.md). Run `./gradlew :apps:epistola:test --tests "*InlineStyleHygieneTest*"` before committing. Use CSS classes for static styles, data attributes for state-driven styling, and CSS custom properties for computed values. See [`docs/style-hygiene.md`](docs/style-hygiene.md) for the full pattern reference with examples.
