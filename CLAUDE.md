...
16. **Follow inline style hygiene rules** - All inline `style="..."`, `classList.*`, and `var(--ep-*, <fallback>)` are enforced by [`InlineStyleHygieneTest`](docs/style-hygiene.md). Run `./gradlew :apps:epistola:test --tests "*InlineStyleHygieneTest*"` before committing. Use contextual/component- or page-scoped CSS classes for static styling, data attributes for state-driven styling, and CSS custom properties only for computed values. In the app UI, do not introduce or extend a generic utility-class layer; prefer named classes that describe the component or page role. Do not put app UI CSS in template-local `<style>` elements; keep it in the app stylesheet unless there is a deliberate documented exception. For JS-driven visibility, prefer `data-hidden="true|false"` selectors over class toggles. See [`docs/style-hygiene.md`](docs/style-hygiene.md) for the full pattern reference with examples.
