...
16. **Follow inline style hygiene rules** - All inline `style="..."`, `classList.*`, and `var(--ep-*, <fallback>)` are enforced by [`InlineStyleHygieneTest`](docs/style-hygiene.md). Run `./gradlew :apps:epistola:test --tests "*InlineStyleHygieneTest*"` before committing. Use design-system classes (`ep-mt-*`, `ep-text-*`, `ep-btn`, etc.) when they fit; for app-specific styling not covered by the design system, add contextual classes to corresponding folder in `apps/epistola/src/main/resources/static/css/templates`. Use data attributes for state-driven styling and CSS custom properties only for computed values. Do not put app UI CSS in template-local `<style>` elements. For JS-driven visibility, prefer `data-hidden="true|false"` selectors over class toggles. See [`docs/style-hygiene.md`](docs/style-hygiene.md) for the full pattern reference with examples.
