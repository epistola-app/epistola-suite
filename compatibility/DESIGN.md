...
degradation). Step 6's **render** is done (`render.sh` → `MATRIX.md`, posted to the
CI job summary). Remaining: step 5 (plugin) and step 6's **aggregate** (multi-source
feeds) — see [Execution plan](#execution-plan-order).
...
6. **Aggregate + render** (D6). **Render** ✅ **DONE**: `render.sh` turns
   ...
   job summary. **Aggregate** (multi-source feeds → one matrix) and the aggregator's
   home are still open, deferred until the plugin/at-rest feeds exist.
