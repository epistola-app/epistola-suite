 ...
 * include feature modules like audit/feedback/loadtest — so their tenant tables aren't even present
 * there. This test boots the full app and asserts every module's tenant-scoped tables are classified
 * by a [app.epistola.suite.backup.TenantBackupTableContributor], so a module that adds a tenant table
...
class TenantBackupClassificationAppTest : BaseIntegrationTest() {
    ...
    fun `every module's tenant-scoped tables are classified in the full app`() {
        jdbi.useHandle<Exception> { handle ->
            ...
            // Excluded module tables are NOT backed up...
            assertThat(backedUp).doesNotContain("audit_log", "load_test_runs")
            // ...and feedback (included) IS.
