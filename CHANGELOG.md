...
- **Horizontal-scaling Phase 1 starts with an always-on Suite node registry.** A new design doc, [`docs/horizontal-scaling-phase1.md`](docs/horizontal-scaling-phase1.md), scopes the first cluster-runtime work to node presence, sticky timer events, durable processes, cache invalidation fanout, and future capability-aware renderer nodes — explicitly leaving distributed command routing out of Phase 1. The first implemented slice adds `suite_nodes`, `epistola.cluster.*` configuration, a `SuiteNodeRegistry`, and a heartbeat scheduler so every replica records its `NodeIdentity`, capabilities, version, join time, and last-seen time in Postgres. A single deployment is treated as a one-node cluster rather than a disabled mode, with default heartbeats every 2s and stale detection after 10s.
