...
- **[user]** feat(catalog): **ZIP imports of an outdated catalog schema version are now rejected.** A catalog ZIP is a source-less, one-shot transport — it cannot be re-fetched and stored content is not migrated in place — so importing a ZIP whose catalog schema version the migrator cannot bring to current is blocked (`CatalogSchemaTooOldException` → "re-export the catalog from a current source") rather than bound as-is under the migrator's transitional leniency. The check is on the migrated manifest version, so a future migration chain that upgrades the payload is not blocked; only the un-upgradable case is. Covers the web and REST import paths (both share `ImportCatalogZip`). URL subscriptions stay lenient and surface the out-of-sync warning instead.
