...
- **Catalog snapshot sync + backups via epistola-hub (`modules/epistola-support-backups`).** A new module that snapshots all of a tenant's catalogs and syncs them to epistola-hub. The snapshot is the shared primitive behind backups (retained, restorable) and the separate Upgrading/compatibility feature (the company side tests those snapshots against unreleased Epistola versions).
  - `BuildTenantSnapshot` packages every catalog via the existing `ExportCatalogZip` (byte-for-byte) into one archive with a `snapshot.json` manifest, and rolls the per-catalog content fingerprints into one tenant fingerprint. It records the **running Suite version** (from `BuildProperties`) on the snapshot — the baseline the compatibility check tests against — shown in the Backups table. The bootstrap-managed **system catalog is excluded** (it is reinstalled from the classpath, not tenant data); the tenant-owned `default` catalog and any subscribed catalogs are included.
  - `BackupSyncPort` + `HubBackupSyncAdapter` (client-streaming upload / server-streaming download over the hub `CatalogSyncService`) + a no-op fallback. Wired to the hub only when `epistola.support.enabled=true`.
  - `CatalogBackupService` uploads a snapshot, skipping the upload when the rolled-up fingerprint is unchanged since the last backup (cached in `app_metadata`); the hub also dedups by fingerprint.
  - `BackupScheduler` runs a daily backup for every tenant with the `support-backups` feature toggle enabled, under a `SchedulerLock` advisory lock (key `CATALOG_BACKUP`) and a per-tenant system principal. Active only when `epistola.support.backups.scheduled.enabled=true` (cron `epistola.support.backups.cron`, default 02:00).
  - `RestoreTenantSnapshot` restores a tenant from a snapshot — **destructive**, in one transaction: it purges the tenant's catalogs, reinstalls the current system catalog, and re-imports each snapshot catalog in cross-catalog dependency order with REPLACE semantics. `ImportCatalogZip` gained a `validateCrossCatalogDeps` flag (set `false` during restore — the snapshot is self-consistent and imported atomically; DB FKs still enforce integrity). The destructive wipe lives in a new core `PurgeTenantCatalogs` command (core owns the catalog FK topology): it nulls the tenant's `default_theme_key` to release the `fk_tenants_default_theme` FK, deletes `variant_attribute_definitions` (the attribute→code-list `RESTRICT` FK), then deletes catalogs; the restore re-applies the prior default theme afterwards. Restore now validates the archive (schema version, tenant match, all catalog bytes present) **before** the wipe.
- **Upgrading (compatibility) is its own feature/module (`modules/epistola-support-upgrading`).** Backups and Upgrading are separate features that both rely on the snapshot sync, so Upgrading lives in its own module with its own `CompatibilitySyncPort` (read-only) + hub adapter, depending on `epistola-support` (not on backups). The daily `BackupScheduler` uploads snapshots when **either** `support-backups` or `support-upgrading` is enabled, so Upgrading works without Backups.
...
- **Support → Backups and Support → Upgrading pages.** The Backups page lists a tenant's snapshots (captured time, suite version, catalog count, size, latest badge), with a "Back up now" action and a restore-with-confirmation action; the Upgrading page shows compatibility-check results fetched live from the hub, grouped by the Epistola version tested. Each is gated for visibility by its own toggle (desktop + mobile nav under "Support") and per-action by `TENANT_SETTINGS`. When the installation isn't entitled (`HubEntitlementDeniedException`) or hasn't finished registering, the pages render an informational state instead of an error.
