...
- **Catalog wire-format schema migrations (EF-style), framework only.** Catalog imports now run an ordered, JSON-tree migration chain over the wire payload _before_ binding it to the typed protocol model, so an export produced at an older `schemaVersion` can be mechanically upgraded to the current shape on import — the same discipline EF Core migrations give a database. New `app.epistola.suite.catalog.migrations` package: `CatalogSchemaMigration` (one step, `from → from+1`, pure `JsonNode` transforms for manifest and resource-detail), `MigrationContext`, and `CatalogSchemaMigrator`, which validates the chain is contiguous and total at startup (Flyway-like; a malformed chain fails app start) and applies the version gate. The migrator is the single chokepoint for both transports — invoked inside `CatalogClient.fetchManifest` (covering every remote consumer: install, browse, upgrade-check, fingerprint) and directly in `ImportCatalogZipHandler` for the ZIP path. **No migrations exist yet** — `CATALOG_PART_SCHEMAS` defines a `baseline == current` window per part (manifest and each resource type), so every chain is empty. Each part is gated independently by its **own** `schemaVersion`: a part newer than this instance is rejected (`CatalogSchemaTooNewException`); a sub-current part binds as-is (transitional — with an empty chain there is nothing to upgrade to, and pre-versioning stamps are unreliable, so strict `TooOld` enforcement + stamp normalisation land with that part's first real migration). All gate exceptions extend `IllegalArgumentException`, so existing import error paths surface them as 400. Architecture and rationale in [ADR 0007](docs/adr/0007-catalog-wire-format-migrations.md); roadmap in [`docs/plans/catalog-wire-format-migrations.md`](docs/plans/catalog-wire-format-migrations.md).
- **[dev]** **Catalog wire-format schema migrations (EF-style, per-part) — with the first real migration (stencil v1 → v2).** Catalog imports run an ordered, JSON-tree migration chain over the wire payload _before_ binding it to the typed protocol model, so an export produced at an older `schemaVersion` is mechanically upgraded to the current shape on import — the same discipline EF Core migrations give a database. **Per-part versioning** ([ADR 0006](docs/adr/0006-catalog-wire-format-migrations.md)): the manifest and each resource type (`CatalogPart`) are versioned independently — each owns a `PartSchemaWindow(baseline, current)` in `CATALOG_PART_SCHEMAS` and its own chain. A `CatalogSchemaMigration` declares its `part` + `from`, and `CatalogSchemaMigrator` validates **each part's** chain is contiguous and total at startup (Flyway-like; a malformed chain fails app start) and gates each part by **its own** `schemaVersion` (`> current` → `CatalogSchemaTooNewException`, `< baseline` → `CatalogSchemaTooOldException`; all extend `IllegalArgumentException` → 400). The migrator is the single chokepoint for both transports — the manifest **and resource details** are migrated at `CatalogClient` (covering install / browse / upgrade-check / fingerprint) and in `ImportCatalogZipHandler` (ZIP). Export stamps each detail with its part's `current` (`CatalogContentBuilder`). **First migration:** `StencilV1ToV2RequireVersionMigration` (stencil part `baseline 1 → current 2`) — a pre-`0.6.0` stencil that carried content with **no `version`** (ADR 0003) is assigned `version = 1` on import and installed cleanly, instead of failing with "missing version, please re-export". Roadmap: [`docs/plans/catalog-wire-format-migrations.md`](docs/plans/catalog-wire-format-migrations.md); per-part contracts: [`docs/exchange/`](docs/exchange/README.md).
