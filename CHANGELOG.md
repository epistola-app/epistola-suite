...
- **[dev]** test(ui): **Page-header quote guardrail.** A bare ASCII apostrophe or double-quote inside an inline `page-header(...)` literal breaks Thymeleaf's fragment-call parser — the build stayed green but the page 500'd at runtime, and no escaping survives that grammar. `PageHeaderUsageTest` now parses every call site with Thymeleaf's own assignation-sequence parser and fails `unitTest` on a bad quote, with a message that points at the fix (a typographic `’`/`“ ”`, or a `${...}`/`#{...}` value). The Defaults page description now uses a natural apostrophe as the worked example.
