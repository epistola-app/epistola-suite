...
- **Support → Backups and Support → Upgrading pages.** The Backups page lists a tenant's snapshots (captured time, suite version, catalog count, size, latest badge), with a "Back up now" action and a restore-with-confirmation action; the Upgrading page shows compatibility-check results fetched live from the hub, grouped by the Epistola version tested. Each is gated for visibility by its own toggle (desktop + mobile nav under "Support") and per-action by `TENANT_SETTINGS`. When the installation isn't entitled (`HubEntitlementDeniedException`) or hasn't finished registering, the pages render an informational state instead of an error. "Back up now" reports the real outcome: a fresh upload ("Backup completed") vs. dedup when the tenant's catalogs are unchanged ("No changes since the last backup — nothing new to store"), so an unchanged re-run no longer looks like a silent failure.
