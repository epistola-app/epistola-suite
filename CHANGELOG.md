...
- **Catalog wire-format schema migrations (EF-style), framework only.** Catalog imports now run an ordered, JSON-tree migration chain over the wire payload _before_ binding it to the typed protocol model, so an export produced at an older `schemaVersion` can be mechanically upgraded to the current shape on import — the same discipline EF Core migrations give a database. New `app.epistola.suite.catalog.migrations` package: `CatalogSchemaMigration` (one step, `from → from+1`, pure `JsonNode` transforms for manifest and resource-detail), `MigrationContext`, and `CatalogSchemaMigrator`, which validates the chain is contiguous and total at startup (Flyway-like; a malformed chain fails app start) and applies the version gate. The migrator is the single chokepoint for both transports — invoked inside `CatalogClient.fetchManifest` (covering every remote consumer: install, browse, upgrade-check, fingerprint) and directly in `ImportCatalogZipHandler` for the ZIP path. **No migrations exist yet** (`CATALOG_MANIFEST_BASELINE_SCHEMA_VERSION == CATALOG_MANIFEST_SCHEMA_VERSION == 4`): a payload newer than this instance is rejected (`CatalogSchemaTooNewException`); a sub-current payload binds as-is (transitional — detail `schemaVersion` stamps have historically drifted and are not trusted, so the manifest version is authoritative and strict `TooOld` enforcement + stamp normalisation land with the first real migration). All gate exceptions extend `IllegalArgumentException`, so existing import error paths surface them as 400. Architecture and rationale in [ADR 0006](docs/adr/0006-catalog-wire-format-migrations.md); roadmap in [`docs/plans/catalog-wire-format-migrations.md`](docs/plans/catalog-wire-format-migrations.md).
- **At-rest content migration (framework only), the in-database counterpart to the wire migrator.** Already-stored content blobs are now upgraded in place at startup, reaching content the wire path cannot — AUTHORED / user-authored resources have no source URL to re-fetch, so only an in-place pass migrates them when a new app version boots against an existing database. Versioning is **one global counter** (modelled on Flyway/EF: the app code is always current, so every stored blob is at one content version), held under a single `app_metadata` key (`catalog.content.schema_version`) — no per-row column, no DDL. `CatalogSchemaMigration` gains a `migrateContentBlob(blobType, JsonNode, ctx)` step keyed by **content shape** (`ContentBlobType`: e.g. one `TEMPLATE_DOCUMENT` step migrates both `template_model` and stencil `content`). On boot, if the counter is below current, `AtRestContentMigrator` runs the **same** registered chain over every bucket-C blob (`themes`, `template_versions`, `stencil_versions`, `contract_versions`) and advances the counter — all in one transaction, so a failure rolls back and the next boot retries with no double-apply. Because the counter cannot tell a freshly-written current blob from a lagging one, `AtRestContentMigrationBootstrap` runs **before** the content-writing bootstraps (`DemoLoader`, `SystemCatalogBootstrap`). Reuses the wire migrator's version gate (too-new / too-old / transitional). **No migrations exist yet** (`baseline == current`), so the pass is a cheap no-op until the first real content change. Only `blockStylePresets` and `dataExamples` diverge between wire and storage today, and field-addressed transforms tolerate that, so no canonicalization was needed. Architecture and rationale in [ADR 0007](docs/adr/0007-at-rest-resource-migration.md).
- **First real catalog wire migration: v3 → v4, and the resource-detail path wired.** `CATALOG_MANIFEST_BASELINE_SCHEMA_VERSION` drops to `3` and `CatalogSchemaMigrationV3ToV4` becomes the first registered step. The only non-additive change at this boundary is that `StencilResource.version` became a required `int` (epistola-model 0.6.0, [ADR 0003](docs/adr/0003-stencil-version-in-export.md)); the step injects `version = 1` into stencil resource details that predate the field, so a genuine v3 catalog now imports instead of failing the pre-scan with "missing version". The manifest's v4 additions (`release.fingerprint`, 3-part SemVer) are nullable / free-form and bind from a v3 manifest as-is, so the manifest transform is identity. This also lands the **resource-detail migration path** that ADR 0006 deferred: `CatalogSchemaMigrator.migrateAndBindManifest` now returns the detected source version (`MigratedManifest`), threaded into a new `migrateAndBindResourceDetail` at **both** transports — the ZIP path (`ImportCatalogZipHandler`) and the remote path (`CatalogClient.fetchResourceDetail`, with the version flowing through `InstallFromCatalog` / `DependencyResolver` / `PreviewInstall`) — so ZIP and remote share one migration path (no drift). A v4 catalog still requires `version`; payloads below the baseline are rejected as too old. Each step also appends a visible **"migratie naar versie 4"** text block to every template on import — a marker that the migration ran (see `TemplateNoticeInjection`).
- **Catalog wire migration extended down to v2 (additive boundary).** `CATALOG_MANIFEST_BASELINE_SCHEMA_VERSION` drops to `2` and `CatalogSchemaMigrationV2ToV3` registers: the v2 → v3 boundary was purely additive (epistola-model 0.4.0 added `CodeListResource`, `DependencyRef.CodeList`, and the optional `AttributeResource.codeListBinding`), so a v2 attribute keeps its inline `allowedValues` and simply lacks `codeListBinding` — no content reshape is required to bind. The step accepts v2 catalogs, which then flow through the chain `2 → 3 → 4` (the `3 → 4` step injects the missing stencil `version`), and appends a **"migratie naar versie 3"** marker text block to every template on import. Converting inline `allowedValues` into a `codeListBinding` would be data loss, not a migration, so it is deliberately not done. Payloads below v2 are rejected as too old.
- **At-rest migrations now transform stored content, and only for AUTHORED catalogs.** The wire migration steps gained `migrateContentBlob` implementations (sharing the notice helper with the wire path), so the startup `AtRestContentMigrator` — which previously called an unimplemented method and silently advanced the counter without touching content — now applies each version's transform to already-stored blobs. `ContentBlobType` splits `template_model` from stencil `content` so the template-only notice targets the right blob (matching the wire path's per-resource-type behaviour). The pass now scans **AUTHORED catalogs only**: SUBSCRIBED content is source-managed and re-migrated via the wire path on upgrade, so mutating it locally would only drift it from source until the next refresh.
- **Cluster scheduling is hardened against slow handlers (multi-node safety).** Three changes remove a duplicate-execution window that opened when a scheduled-task or timer handler ran longer than the cluster idle/lease timeouts (e.g. tenant backups, snapshot uploads, hub calls): (1) the node **heartbeat no longer shares the Spring `@Scheduled` pool** with handler dispatch, so a blocked handler can no longer starve liveness, falsely mark a healthy node stale, and shift routing-key ownership; (2) both pollers **renew the lease while a handler runs** (`ClusterLeaseRenewer` + `renewLeases` on each registry), so a legitimately long handler is not reclaimed and re-run by another node — even across a membership change; (3) **per-`taskType`/`timerType` handler metrics** (`epistola.scheduled.task.duration` tagged `cluster-scheduled-task:<type>` / `cluster-timer:<type>`) make a single slow or failing task visible in the fleet instead of being hidden inside the poll-cycle timer. The heartbeat and both lease renewers run on **one shared single-thread maintenance executor** (`ClusterMaintenanceExecutor`) — none of them run handler work, so they cannot starve each other — and the liveness queries (heartbeat + `renewLeases`) use a short query timeout so a degraded database fails them fast and they retry next tick instead of wedging that thread. Every native scheduled-task registration now declares its `executionScope` explicitly. Adds unit coverage for the schedule calculator (backoff cap/ceiling, fixed-rate coalesce vs catch-up, cron DST/zone, error paths) and integration coverage for reclaim idempotency (a stale owner's `complete`/`fail` is a no-op) and lease renewal blocking reclaim.
- **Native scheduled tasks now cover singleton and all-node work.** Scheduled task definitions gained an `execution_scope`: `single_owner` keeps the existing once-per-cluster lease behavior, while `each_capable_node` stores per-node runtime state in `cluster_tasks_scheduled_node_state` so every active capable node can run its own occurrence. Business `@Scheduled` usages were migrated to native scheduled tasks: feedback retry/poll, backups, upgrading snapshots, stale job recovery, load-test cleanup, document/load-test poller wakeups, hub health checks, entitlement refresh, and installation stats. The Operations → Cluster page now shows task execution scope, browser-local date/time values for due timestamps, and a nested per-node status table under each all-node scheduled task; raw Spring scheduling remains only for cluster heartbeat and native timer/scheduled-task poller wakeups.
- **Application time now flows through mediator context.** `MediatorContext` now binds the mediator, active `Clock`, and optional principal; `SpringMediator` creates that scope for direct command/query dispatch; and `EpistolaClock` resolves time from mediator context before falling back to UTC. Immediate scheduled work binds with `MediatorContext.runWithMediator(mediator)`, while executor handoffs use `MediatorContext.runnable(...)` or `MediatorContext.callable(...)` to capture and re-bind the context at the boundary. Obvious production JVM `now()` call sites across API responses, API keys, consumer status, assets/content storage, document generation, support snapshots/entitlements, and load-test maintenance now use `EpistolaClock`; DB-owned `NOW()/now()` paths remain on the database clock.
- **Agent guidance now documents mediator-owned application time.** `CLAUDE.md` records the required `EpistolaClock`/`MediatorContext` rules for application time, thread handoffs, database-owned time, and tests; `AGENTS.md` now directs agents to read `CLAUDE.md` before making changes.
- **Immediate scheduler scopes now use `MediatorContext.runWithMediator`.** Scheduler-style entrypoints bind mediator context directly when they execute on the current thread, while `MediatorContext.runnable(...)` and `MediatorContext.callable(...)` handle executor handoffs.
- **Mediator executor handoffs now use `MediatorContext` directly.** `MediatorExecutionContext` is internal implementation state; production call sites use `MediatorContext.runnable(...)` / `.callable(...)` so clock and principal propagation stays under the mediator context API.
- **Timer documentation now reflects the current mediator scope boundary.** `docs/timers.md` now describes same-thread timer and scheduled-task pollers as `MediatorContext.runWithMediator` scopes and reserves `MediatorContext.runnable(...)` / `MediatorContext.callable(...)` for executor/callback handoffs.
- **Minimal eventing design documented.** Added [`docs/minimal-eventing.md`](docs/minimal-eventing.md), describing the proposed PostgreSQL-backed event stream, tenant-aware event envelope, all-node tailing model, fast envelope filtering, poller wakeups for timers/scheduled tasks/cluster membership, and the boundary between phase-1 wakeups and future durable saga processing.
- **Cluster timer and scheduled-task tests cover failure and contention paths.** Timer/schedule handler registration now fails fast when duplicate `timerType` or `taskType` beans are present. Focused tests cover missing handlers being retried instead of dropped, duplicate handler detection, simulated two-node lease contention for timers and scheduled tasks, and startup scheduled-task registration behavior. The scenario test DSL now captures the active `EpistolaClock` and exposes `withClock { ... }` for callbacks that execute on worker threads.
...
- **All-node scheduled tasks now show an effective next due time in Operations.** The Cluster dashboard parent row now uses the earliest per-node `next_due_at` for `each_capable_node` tasks when node state exists, avoiding stale definition seed timestamps that made healthy per-node schedules look overdue.
