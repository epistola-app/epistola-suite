...
- **Tenant default locale + feature toggles on the settings page (Phase 1: configuration + override UI).** A new application-wide `epistola.i18n.default-locale` property (default `en-US`, bound through `LocaleProperties`) is the suite-wide fallback locale; tenants can override it from a new `/tenants/{tenantId}/settings` page that also surfaces the per-tenant feature toggles (`POST /tenants/{id}/settings/features` reuses the existing `SaveFeatureToggle` command and `KnownFeatures.all`). The standalone `/tenants/{id}/features` page and its nav entry remain untouched — settings is an additional surface, not a replacement. Storage: nullable `tenants.default_locale` column (`V20260519142326__core_tenant_default_locale.sql`; NULL = "use the app default"), with a defensive BCP-47 shape CHECK. Authoritative validation in the `SetTenantDefaultLocale` command runs against the existing `system.bcp-47` code list via `CodeListEntryExists` — pick from a curated list, no free-form typos. A new `TenantLocaleResolver` (`tenant.defaultLocale ?: properties.defaultLocale`) is the single read seam Phase 2 will consume — deliberately not named `LocaleResolver`, because that bean name collides with Spring MVC's own `org.springframework.web.servlet.LocaleResolver` lookup in `RequestMappingHandlerAdapter`. The tenant home page gains a **Settings** quick action linking to the new page; HTMX form rerenders the fragment on save/error (no 3xx), fires `tenantSettingsUpdated` on success. **Phase 2 (deferred, separate PR):** wiring `LocaleResolver` into template/document rendering so dates and numbers actually pick up the locale; surfacing `defaultLocale` on REST `Tenant` DTOs (the existing `updateTenant` is still a 404 stub) and adding a tenant-metadata MCP read tool. Phase 1 ships no demo-catalog update — a tenant settings field has no per-tenant catalog representation; the locale-driven formatting demo lands with Phase 2.
