...
The next release makes the in-app changelog filterable by audience, change type, and scope, and persists application logs to a per-tenant, weekly-pruned table with an Operations → Logs viewer. It also brings locale-aware date and number formatting to the editor and PDF output, durable cluster scheduling, and a range of rendering and feedback fixes.
...
- **[dev]** feat(cluster): **Code-defined cluster scheduled tasks are retired and purged automatically when they disappear from code.** A durable `cluster_tasks_scheduled` row used to outlive its `ClusterScheduledTaskDefinition` forever — the scheduler kept polling it, found no handler, and recorded an error + backoff retry on every cycle, so a removed schedule looked like a permanently broken task. Detection is now **registration-based across the active fleet**: each node, at startup, records itself in a new `cluster_scheduled_task_registrations` table (`task_key`, `node_id`, `build_version`, `registered_at`) as vouching for exactly the definitions it carries, and prunes its rows for definitions it no longer carries. A new native `single_owner` task `core.scheduled-task-reconciliation` (`ClusterScheduledTaskReconciler`, hourly) retires a `management_mode = 'code'` row only when **no node carrying the definition has been seen within the grace window** — liveness is judged by the registering node's heartbeat-maintained `cluster_nodes.last_seen_at` (not the startup `registered_at`, which would go stale for a long-running node), so a node must be unseen for `epistola.cluster.scheduled-tasks.reconciliation-grace-period-ms` (default 15 min) before its schedules become removable. This is robust across long-running-node restarts and rolling deploys (old nodes keep their schedules alive until they actually leave), with no build-version coupling. Retirement is a soft step (`retired_at`/`retirement_reason`, disabled, lease cleared) that stays visible on the Operations → Cluster page as a "retired" status; rows are hard-deleted after `epistola.cluster.scheduled-tasks.retired-retention-ms` (default 7 days), cascading per-node state and registrations. Re-adding a definition to code reclaims and un-retires it. Manual/operator rows (`management_mode = 'manual'`) are never auto-retired. Relatedly, a scheduled task with **no registered handler** is no longer treated as a failure: the dispatch path folds in the orphan check — if no node carrying the definition has been seen within the grace window it is **soft-retired inline** the moment it fires (no waiting for the periodic reconciler); otherwise a node still holds it (rolling-deploy in-between state, or a non-handler node claimed it by capability) and the scheduler quietly advances `next_due_at` (no error state, no tight reclaim loop) so a holding node can run it. The periodic reconciler remains the backstop for orphans that never fire. See [`docs/timers.md`](docs/timers.md#scheduled-task-lifecycle).
