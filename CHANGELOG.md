...
- **Catalog backups + compatibility via epistola-hub (`modules/epistola-support-backups`).** A new module that snapshots all of a tenant's catalogs and syncs them to epistola-hub — the shared primitive behind backups (retained, restorable) and compatibility checks (the company side tests a tenant's catalogs against unreleased Epistola versions).
  ...
  - `RestoreTenantSnapshot` restores a tenant from a snapshot — **destructive**, in one transaction: it deletes `variant_attribute_definitions` first (the attribute→code-list cross-catalog FK is `ON DELETE RESTRICT`) then all catalogs, reinstalls the current system catalog, and re-imports each snapshot catalog in cross-catalog dependency order with REPLACE semantics. `ImportCatalogZip` gained a `validateCrossCatalogDeps` flag (set `false` during restore — the snapshot is self-consistent and imported atomically; DB FKs still enforce integrity).
