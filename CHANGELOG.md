...
Upcoming changes focus on CI stability, security hardening, and clearer development guardrails.

- **[user]** feat(ui): **Every "create new" form now opens in a modal dialog instead of its own page, and create dialogs can be deep-linked.** Templates, themes, API keys, environments, stencils, attributes, fonts, assets, code lists and load tests all load their create form into a dialog (HTMX) from the list page — you no longer navigate to a separate `…/new` page. Validation errors re-render inside the still-open dialog, a successful create takes you to the new item (or, for API keys, reveals the one-time secret in the dialog), and the cascading template→variant selects on the load-test form work inside the dialog. The shared wiring is a single `#dialog-host` in the shell plus open/close handling in `fragments/htmx`; the create form is dialog-only, so a direct `GET …/new` (no-JS / deep link) now redirects to the list rather than rendering a standalone page. On top of that, a create dialog can opt into being **URL-addressable**: opening it puts `?create` in the address bar (preserving any filter the list already has — e.g. the catalog filter on the templates list), so you can bookmark or share the link, reload without losing the dialog, and press Back to close it (Cancel/ESC strip `?create` back out so the URL never lies about what's on screen). It's a per-entity opt-in with entity-agnostic wiring (the dialog names its URL param via `data-dialog-param`): the seven create forms — **templates, themes, API keys, environments, stencils, attributes, code lists** — use `?create`, and the two upload forms — **fonts, assets** — use `?upload`. The **Catalogs** page was also converted to this dialog model (from its old in-page dialog + out-of-band list swap; creating a catalog now lands you on the new catalog's browse page) and deep-links via `?create`. **Load tests** deep-link via `?create` too — only the dialog's open state is shared, not the cascade selections. **Every create dialog in the app is now deep-linkable.** See [`docs/htmx.md`](docs/htmx.md).
- **[dev]** refactor(ui): **Unified create-form validation-error handling.** Every create form now draws its error border from a single CSS rule (`.form-group:has(.form-error[data-error='true'])`) keyed off a `data-error` attribute on the error span, replacing the per-field `th:classappend="… 'error'"` `.form-group.error` toggle (including the shared `fragments/form-fields.html` macros). The **font and asset upload** dialogs no longer return a JSON 400 parsed by a bespoke inline `htmx:responseError` script — on a validation error the handler now answers **200** with out-of-band (`hx-swap-oob`) error spans (per-field for named fields, a general region for face/file problems), swapped via `reswap(HxSwap.NONE)` so the **chosen file survives** while the error renders inline. The shared rule: re-render the largest region whose state the server can reconstruct — the whole form for text forms, just the error spans for file/cascade forms. The asset endpoint keeps its **JSON-400** contract for the editor's non-HTMX `uploadAsset` caller (it branches on `request.isHtmx`). See [`docs/htmx.md`](docs/htmx.md).
- **[dev]** refactor(ui): **Centralized general/server error rendering for create dialogs.** General (non-field) errors are no longer hand-rendered per form — handlers **throw** (`FormInputException`, or any domain exception) and the MVC error handler `UiHandlerExceptionResolver` renders them **through the view layer** (it returns a `ModelAndView`; the HTML comes from a `fragments/dialog :: dialogError` fragment, so no component hand-builds markup — `UiExceptionFilter` stays the last-resort net for what escapes the dispatch, and both share one `resolveUiError` mapping). It content-negotiates one envelope: an HTMX form that declares a region via the `X-Epistola-Error-Region` header (set on the create `<dialog>` via `hx-headers`, absent on every other HTMX flow → no blast radius) gets **200 + `HX-Reswap: none` + an out-of-band swap** of an alert into a shared `#dialog-error` region _inside_ the modal; a data caller (`Accept: application/json`, e.g. the editor) gets **RFC 9457 problem+json** whose `detail` carries the message. This kills the "500 banner behind the open modal" failure mode (the global `htmx:responseError` net inserted it into `#main-content`). fonts/assets drop their JSON-400 branch and per-form `error-general` span; load-test drops its `#form-error` region and routes operational errors to the shared region; **text forms needed no handler change** — they already let operational exceptions propagate. Field errors are unchanged (still rendered by the handler as data, next to their inputs). A read-only-catalog rejection now returns **409** (the problem type's canonical status, matching the REST API) instead of the UI's old 403. See [ADR 0008](docs/adr/0008-create-form-validation-errors.md) and [`docs/htmx.md`](docs/htmx.md).
- **[user]** fix(ui): **Load-test create errors now appear under their field.** In the load-test dialog, invalid JSON and a missing or invalid template/variant/version now render as an inline error **under the relevant field** (a per-field out-of-band span answered with `HX-Reswap: none`, so your cascade selections and typed JSON survive) instead of a general alert at the top of the dialog. The shared `#dialog-error` card is now reserved for **non-field, operational** failures — e.g. the backend rejecting the start request. This applies the ADR 0008 "field problems render next to the field; only non-field/operational problems use the shared card" rule to load-test, which had been throwing every error to the card. See [ADR 0008](docs/adr/0008-create-form-validation-errors.md) and [`docs/htmx.md`](docs/htmx.md).
- **[user]** fix(ui): **Create-dialog field errors no longer silently vanish, and per-field spans are now a build-enforced rule.** Five create dialogs (templates, themes, stencils, attributes, code lists) keyed a validation error (e.g. "Catalog is required") to a field that had **no error span** in the template, so the message was silently dropped instead of shown next to the field. Those fields now render their error inline. Going forward this is a convention enforced at build time: a new `CreateDialogErrorConventionTest` (in `unitTest`) fails the build unless **every** create dialog (a) includes the shared `#dialog-error` general card + the `X-Epistola-Error-Region` header, and (b) gives every user-editable payload field its own per-field error span. The `htmx-form` skill and [ADR 0008](docs/adr/0008-create-form-validation-errors.md) document the shape; non-payload controls (file inputs, radio/checkbox groups, a cascade-only helper select) are exempt.
