...
- **DB migrations can run as a separate, explicit step (issue #431).** `main()` now detects `EPISTOLA_RUN_MODE=migrate` (or `--migrate`) before Spring starts and hands off to a new `MigrationLauncher`, which boots an isolated context (only the datasource + Flyway auto-config + the existing `FlywayConfig` — no web server, schedulers, demo loader or catalog bootstrap), runs `flyway.migrate()`, and exits 0/1. It reuses the same image and the same `application.yaml`, so Flyway locations and `SPRING_DATASOURCE_*` are identical to the embedded path (no migrate/runtime drift); it always forces `clean-disabled: true` (a failed migration is a non-zero exit, never a destructive reset). New `epistola.migration.mode` (`migrate` | `validate`), branched in `FlywayConfig`'s `FlywayMigrationStrategy`: `migrate` (default — local/dev embedded, unchanged) runs migrations at boot; `validate` (set by the `prod` profile, and by the Helm chart in `job`/`initContainer` modes via `EPISTOLA_MIGRATION_MODE`) makes app pods only validate the schema and **fail fast if it is behind — never migrate at boot**. The Helm chart gains a `migration` block (`mode: job` default — a `pre-install`/`pre-upgrade` hook Job; `initContainer`; or `embedded`) — see [`docs/deployment.md`](docs/deployment.md) and [`charts/epistola/CHANGELOG.md`](charts/epistola/CHANGELOG.md). Runtime partition DDL is unchanged here (the runtime role still needs DDL); the SECURITY DEFINER + two-role split is tracked separately (#438).
