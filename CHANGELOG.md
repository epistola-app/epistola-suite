...
- **[dev]** feat(catalog): **First catalog wire-format migration — an example `v3 → v4` step.** The catalog migration chain now spans `[3, 4]` with one step, `CatalogV3ToV4ExampleMigration` (`CATALOG_BASELINE_SCHEMA_VERSION` drops to `3`; `CATALOG_SCHEMA_VERSION` stays `4`). It upgrades a `v3` catalog — legacy `catalog.title` / `resource.displayName` fields — to `v4` (`name`) on import, defensively and idempotently, while a `v4` catalog binds directly. It is the **reference implementation** for writing a real migration (declare `from`, override `migrateManifest` / `migrateResourceDetail`, register as a `@Component`, bump the version). Unit tests cover the step and the end-to-end gate (`v3` manifest + detail migrate and bind); committed bundled/test catalogs stay at `v4` (within the new `[3, 4]` window, fingerprints unchanged).
