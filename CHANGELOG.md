...
- **`InlineStyleHygieneTest` — automated enforcement of no-inline-styles policy.** 8 checks that fail CI on violations: HTML inline `style="..."`, `th:style` without `--ep-*`, duplicate `class` attributes, CSS/TS `var(--ep-*, <fallback>)`, TS static `style="..."`, TS runtime `style.color/background` (with ProseMirror exemption), and TS `classList.add/remove/toggle/contains`.
- **`stylelint.config.mjs` — CSS linting for the design system.** Enforces no hex colors, no unit values (px/rem/em), and no `.btn` class naming in design-system CSS.
- **Design-system spacing, typography, and display utility classes.** Added `ep-mt-*`, `ep-mb-*`, `ep-p-*`, `ep-text-*`, `ep-font-*`, and `ep-inline` classes to `modules/design-system/components.css` used across 30+ templates.
- **Contextual CSS files for app templates.** Moved app styles from `main.css` and template-local `<style>` blocks into per-page files under `static/css/templates/`.
- **`data-hidden` attribute pattern for JS-driven visibility.** Replaced `ep-hidden` class-based toggling with `data-hidden="true|false"` selectors.
...
- **Replaced all inline `style="..."` in Thymeleaf templates with CSS classes.** ~89 margin/padding, ~52 page-specific, and all inline typography/display/flex styles across 30+ templates moved to design-system utility classes and page-scoped selectors.
- **Replaced dynamic `th:style` expressions with CSS custom properties or `th:classappend`.** Conditional visibility/opacity toggles migrated to `th:classappend`; computed values (tree-indent, progress bar) migrated to `--ep-*` custom property carriers.
- **Replaced intermediate utility-class layer with contextual CSS.** Removed one-off utilities (`ep-hidden`, `ep-inline`, spacing/width/flex helpers) from `main.css`. Remaining templates use page-scoped selectors or `data-hidden` toggles.
- **Migrated 52 `classList` calls to data attributes across 10 editor TypeScript files.** Stencil picker, binding row, parameter bindings, asset picker, table dialog, bubble menu, expression node view, and drag-and-drop components use `element.dataset.*` + CSS `&[data-*]` selectors.
- **Removed ~60 redundant `var(--ep-*, #hex)` fallbacks from 7 editor CSS files.** Design-system tokens are always loaded, making fallbacks dead weight.
- **Replaced JS-controlled `element.style.display` with `.hidden` CSS class in 4 editor components.** Stencil-picker, parameter-bindings, binding-row, and asset-picker dialogs.
- **Corrected `docs/style-hygiene.md` and `CLAUDE.md` item 16.** Rewrote to describe the actual two-tier approach: design-system utility classes first, contextual classes for app-specific needs.
...
- **Duplicate `class` attributes in two templates caused by bulk inline-style removal scripts.** The Group 1 script's regex produced `class="..." class="ep-..."` instead of `class="... ep-..."`, causing Thymeleaf 500 on code-lists/list.html and feedback/submit-form.html.
