...
- **[user]** fix(templates,themes,stencils,attributes,code-lists): **Paginated lists no longer skip or duplicate a row across pages.** When a tenant holds the same template/theme/stencil/attribute id (or code-list slug) in two catalogs — the catalog-override case — the offset-paging tiebreaker was a bare id/slug that is unique only _within_ one catalog, so two rows could tie and one fall outside every page window (invisible) or appear on two pages. The tiebreaker now includes the catalog key, giving a total order across all catalogs. (Environments and API keys were already tenant-unique.)
- **[user]** fix(ui): **A huge `?page=` value no longer 500s a list.** An out-of-range page from a crafted/edited URL computed its SQL `OFFSET` in 32-bit arithmetic, which could overflow to a negative offset (a 500) or a silently wrong page. The offset is now computed in 64-bit, so a large page simply clamps to the last page like any other stale deep link.
- **[user]** fix(mcp): **`list_templates` returns every template, not just the first 50.** The MCP tool used the query's default 50-row window (every sibling list tool already requested all rows), so an assistant could not see templates past the first page in a tenant with 50+; it now returns the full set.
- **[user]** fix(search): **List search matches `%` and `_` literally.** Six list searches (API keys, fonts, themes, stencils, attributes, code lists) bound the term into `ILIKE` without escaping, so a typed `%` or `_` acted as a wildcard (a bare `%` matched everything). All list searches now escape through one shared `ilikeContains` helper.
- **[user]** refactor(catalogs): **New / Subscribe / Import are full pages, like the other lists.** The catalog list's three header actions opened on-page dialogs; they are now `/new`, `/subscribe` and `/import` form pages (matching themes/stencils/fonts), with validation/import errors re-rendered on the page. The catalogs table also reuses the shared sortable-header fragment instead of its own copy.
- **[dev]** perf(fonts): **Fonts list stops issuing one query per font.** Rendering and searching the fonts list fetched each family's faces with a separate query — an unbounded N+1, since the fonts list is unpaginated. A new `ListFontsWithVariants` batches every face into one query (the `ExportFonts` pattern). Also de-duplicated across the list pages: the per-handler htmx wiring (`dataTableResponse`) and offered page sizes, the catalog-filter and sortable-header markup, and the LIKE escaper now live in shared `epistola-web` / `epistola-core` helpers.
- **[user]** feat(catalogs,fonts): **Search + sort on the catalogs and fonts lists.** The catalogs list gains a search box and clickable, sortable column headers (Name, ID, Type, Last Modified); it sorts but does not paginate (the catalog count per tenant is small). The fonts list gains a search box too, keeping its card-grid layout (no sort/pagination). On both pages the search term and (for catalogs) the active sort live in the URL, so the view is bookmarkable, survives a refresh, and is preserved across row actions — a delete/release re-renders honoring the active filter instead of snapping back. This completes the rollout of the reusable `epistola-web` list chrome to every management list page. See [`docs/adr/0007-query-pagination-sorting-filtering.md`](docs/adr/0007-query-pagination-sorting-filtering.md).
- **[user]** feat(templates): **Sortable, paginated templates list.** The templates list now has clickable column headers (Name, Variants, Last Modified) to sort, numbered pagination with a rows-per-page selector (10 / 25 / 50), and a "Showing X–Y of N" summary — so the page stays usable with 100+ templates (it previously showed only the first 50, silently). Sort, search, the catalog filter and the page number all compose and live in the URL, so the view is bookmarkable and survives a refresh — changing any one preserves the others. It is built on a new **reusable server-rendered data-table component** in `epistola-web`: the search box and filters are plain fields of one list `<form>` that submits the whole state together, while the sort headers and pagination are server-built links — so a new filter is just another field, with no per-control wiring. Columns have fixed, truncating widths so long values (a long name, ID, or message) never stretch the table past the page, and on a narrow screen the table scrolls horizontally inside its own wrapper instead of forcing the whole layout wide. The other list pages adopt the same component next. See [`docs/adr/0007-query-pagination-sorting-filtering.md`](docs/adr/0007-query-pagination-sorting-filtering.md).
- **[dev]** feat(templates): **Query-layer pagination and sorting for template summaries.** Added shared, surface-agnostic paging primitives (`SortSpec`, `PageRequest`, `PagedResult`) in `epistola-core` and wired `ListTemplateSummaries` to them: it now returns a `PagedResult` — one windowed page plus the full _filtered_ `total` in a single round trip via `COUNT(*) OVER()` — sorts by a per-query **whitelisted** column (with a `dt.id` tiebreaker for stable offset paging), and clamps an out-of-range page to the last page instead of returning an empty result with a zero total. `PageRequest` fails fast on out-of-range bounds at construction, so junk input can never reach SQL. No user-facing change yet — the reusable sortable/paginated UI table that consumes this lands separately. See [`docs/adr/0007-query-pagination-sorting-filtering.md`](docs/adr/0007-query-pagination-sorting-filtering.md).
