...
- **Cluster timers provide the first durable scheduled-work substrate.** A new `cluster_timers` table stores delayed timer events with optional tenant ownership (`tenant_key` is `NULL` for system timers and cascades on tenant deletion), routing-key affinity, JSON payload, status, lease owner/expiry, attempt count, and error metadata. `ScheduleClusterTimer` and `CancelClusterTimer` are the mediator-facing API for creating and cancelling timers; `ClusterTimerRegistry` schedules, cancels, claims, completes, reschedules, and retries timers using Postgres row leases with `FOR UPDATE SKIP LOCKED`; `ClusterTimerScheduler` polls due timers, keeps execution sticky via rendezvous ownership over active `cluster_nodes`, and dispatches them through the `ClusterTimerHandler` SPI.
- **Cluster scheduled tasks add durable recurring work with node affinity.** A new `cluster_tasks_scheduled` table stores recurring task definitions separately from one-shot timers, including optional tenant ownership, routing-key affinity, cron/fixed-delay/fixed-rate schedule shape, failure/catch-up policy, next due time, lease owner/expiry, and rolling execution metadata. `ClusterScheduledTaskRegistry` upserts definitions, claims due work with Postgres row leases, advances recurrence on success, and records retry/advance behavior on failure; `ClusterScheduledTaskScheduler` polls due tasks and dispatches only tasks owned by the current node in the active `cluster_nodes` ring. The mediator-facing controls are `UpsertClusterScheduledTask`, `EnableClusterScheduledTask`, `DisableClusterScheduledTask`, `TriggerClusterScheduledTaskNow`, and `ListClusterScheduledTasks`. Cluster time now flows through an injectable `Clock`, and tests get a reusable `MutableClock` so timers and schedules can be advanced deterministically instead of relying on Awaitility or wall-clock sleeps. The first migrated jobs are the stale consumer-node reaper and partition maintenance recurring run; partition bootstrap still runs at application startup.
