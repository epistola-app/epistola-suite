...
- **Footer chrome is module-contributed too (`FooterContributor` SPI).** A sibling SPI in `epistola-web` (`app.epistola.suite.htmx.footer`) lets modules inject Thymeleaf fragments into the shell footer; `FooterFragmentResolver` collects them per request (same `UiRequestContext` as nav). The feedback floating-action-button (and console-capture script), previously hardcoded in the host footer behind a `feedbackEnabled` flag, now lives in `epistola-support-feedback` (`templates/feedback/footer-fab.html` + `FeedbackFooterContributor`, gated on `support-feedback`). The host footer no longer references any feature, removing the last feedback coupling from `apps/epistola`.
