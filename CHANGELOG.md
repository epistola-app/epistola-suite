...
- **[dev]** refactor(ui): **Page headers consolidated into a shared fragment.** List, create, and detail pages now render their header through a single `epistola-web/page-header` fragment (required `pageTitle`, optional description, search box, actions, and back link) instead of each template hand-rolling its own `page-header` markup; per-page action buttons move to `epistola-web/page-actions`. The shared chrome lives in the `epistola-web` toolkit module (not the host app), so the support feature modules that consume it no longer depend on `apps/epistola`. The title always carries `data-testid="page-title"`. Header spacing is owned by `.page-header` in the design system, and the generic header deliberately stays chrome-only — page-specific state (read-only notes, metadata rows, status badges, source URLs) renders below it. Visual parity, no behavior change.
