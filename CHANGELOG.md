...
- **Catalog backups + compatibility via epistola-hub (`modules/epistola-support-backups`).** A new module that snapshots all of a tenant's catalogs and syncs them to epistola-hub — the shared primitive behind backups (retained, restorable) and compatibility checks (the company side tests a tenant's catalogs against unreleased Epistola versions).
  - `BuildTenantSnapshot` packages every catalog via the existing `ExportCatalogZip` (byte-for-byte) into one archive with a `snapshot.json` manifest, and rolls the per-catalog content fingerprints into one tenant fingerprint. The bootstrap-managed **system catalog is excluded** (it is reinstalled from the classpath, not tenant data); the tenant-owned `default` catalog and any subscribed catalogs are included.
