...
- **[dev]** refactor(ui): **Visibility convention: `.hidden` class replaces all static `style="display: none"`.** A single `.hidden { display: none; }` utility now controls initial hidden state across 7 templates: the asset-upload file input and preview, the catalog-subscribe credential group, version-comparison iframes and loading spinners, feedback screenshot panels (unsupported/error/preview), the confirm-dialog error bar, and variant attribute-rows. The corresponding JS was updated from raw `element.style.display = 'none'` / `'block'` / `'flex'` to `classList.add('hidden')` / `classList.remove('hidden')`, relying on element or component classes to restore the correct display value (e.g. `.version-comparison-loading` already has `display: flex`, `.ep-row` does the same for the screenshot-actions bar). An iframe `display: block` rule was added to `.version-comparison-frame-container iframe` so removing `hidden` on the comparison iframes restores the correct layout. Dynamic filter/results toggles (`th:style`, `card.style.display` in variant filtering) were intentionally left alone.
- **[dev]** refactor(ui): **`disabled` and `inert` are native platform conventions — no CSS needed.** The `disabled` attribute (already used throughout on `<button>`, `<select>`, `<input>`) and `inert` (zero current usage, forward-looking for modal subtree blocking) are handled by the browser — no design-system class is needed.
