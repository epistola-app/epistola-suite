...
- **Cross-cutting test-run metrics harness.** `modules/testing` now captures test performance on every run with no per-test code, via three seams in `app.epistola.suite.testing.metrics`: a JUnit Platform `TestExecutionListener` (`TestTimingListener`, auto-registered through `META-INF/services`) that times every class and writes a JSON report; a Spring `ApplicationContextInitializer` (`ContextBootCounter`, via `META-INF/spring.factories`) that counts fresh `ApplicationContext` boots (= test-context cache misses, the cost driver behind context fragmentation); and a `@Primary` `Mediator` decorator (`MetricsRecordingMediator`, imported by `IntegrationTestBase`) that times every command/query — including nested dispatches, so `CreateTenant` time includes the `InstallSystemCatalog`/font-import cascade it triggers and the tenant-bootstrap count derives from it. Each test task writes `build/test-metrics/<module>-<task>.json` (wall time, context boots, tenant bootstraps, slowest classes, per-command/-query dispatch profile) and prints a console summary; CI uploads them as the `test-run-metrics` artifact (30-day retention) so suite performance is monitorable over time instead of eyeballed. See [`docs/testing.md`](docs/testing.md#test-run-metrics-cross-cutting).
