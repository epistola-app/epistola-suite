...
- **[dev]** feat(catalog): **Example catalog wire-format migrations — the chain now spans `[3, 5]`.** Two reference migration steps land (`CATALOG_BASELINE_SCHEMA_VERSION = 3`, `CATALOG_SCHEMA_VERSION = 5`) that demonstrate a real content transform: `CatalogV3ToV4ExampleMigration` appends an **"upgrade naar versie 4"** text block to every template (the top-level model and each variant model), and `CatalogV4ToV5Migration` appends **"update naar versie 5"** — both via a shared `injectTemplateNotice` helper, idempotent and a no-op for non-template resources. Together they are the **reference implementations** for writing a real migration (declare `from`, override `migrateResourceDetail` / `migrateManifest`, register as a `@Component`, bump the version). Unit tests cover each step and the end-to-end gate; committed bundled/test catalogs are stamped at the current `v5` (so they bind directly without on-load migration, fingerprints unchanged since `schemaVersion` is excluded).
