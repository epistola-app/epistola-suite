...
- **[user]** fix(editor): **Stencil parameter type names now match the data-contract editor.** The stencil-parameter editor showed its own labels (`String`, `Boolean`, `Date-time`) that disagreed with the data-contract editor's (`Text`, `Yes/No`, `Date & time`) for the same underlying types. The stencil editor now reuses the data-contract editor's `FIELD_TYPE_LABELS` as the single source of truth — in both the type dropdown and the parameter-list badge (which previously showed the raw lowercase type, e.g. `string`) — so the two surfaces always read identically and can't drift apart again. Type values and serialization are unchanged; labels only.
