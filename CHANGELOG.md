...
### Added
- **Style input clear buttons**: Added × clear buttons to color, unit (fontSize, letterSpacing, width), and number (lineHeight) inputs in the inspector. Clicking clears the value to undefined, falling back to component defaults.
- **Border and borderRadius preset editor support**: Fixed [object Object] display bug in theme editor preset section. Added proper rendering for border and borderRadius fields with link mode state tracking.
- **Theme editor layout improvements**: Added content width variables (--ep-content-width-default, --ep-content-width-wide). Theme editor now uses 96rem wide layout. Changed grid from 50-50 to 40-60 split for more preview space. Added thin scrollbars and subtle border separators between sections.
- **Border input linking**: Added "All", "Horizontal", and "Vertical" linking options to the border style editor. Users can now sync border width, style, and color across multiple sides simultaneously. "All" mode links all 4 sides; "Horizontal" links top+bottom and left+right separately; "Vertical" links left+right and top+bottom separately. The linking state is persisted per-block and isolated between different nodes, following the same pattern as margin/padding inputs.
...
- **Border and border-radius support**: Added comprehensive CSS border support with 16 canonical properties (border width/style/color for all 4 sides + border radius for all 4 corners). Editor provides composite controls for both border (width + style + color per side) and border radius (per corner). Supports five border styles: none, solid, dashed, dotted, double. PDF rendering implemented via iText borders (SolidBorder, DashedBorder, DottedBorder, DoubleBorder). Available on text, container, image, columns, conditional, and loop components. Excluded from table/datatable components which use their own grid-border model.
- **Border radius PDF rendering**: Implemented border-radius support in PDF generation using iText's `BorderRadius` class. All four corners (`borderTopLeftRadius`, `borderTopRightRadius`, `borderBottomRightRadius`, `borderBottomLeftRadius`) are now applied to block elements via `setBorderTopLeftRadius()`, `setBorderTopRightRadius()`, etc. methods. Supports px, pt, em, and rem units with automatic conversion to points.
- **Border radius "All" linking**: Added "All" checkbox to the border radius editor. When enabled, all four corners are linked together - changing one updates all. When disabled, each corner can be edited independently. Link state is persisted per-block and isolated between different nodes, following the same pattern as margin/padding inputs.
...
- **Border clear button functionality**: Fixed the "Clear All" button for border and border-radius inputs. Previously, clicking clear would pass an empty object that didn't trigger property deletion, leaving old border values in the document.
- **Component applicable styles updated**: Corrected `applicableStyles` arrays across all component registrations to use the new composite field keys (`border`, `borderRadius`) instead of old individual property keys (`borderWidth`, `borderStyle`, `borderColor`). Also removed CSS border support from table, datatable, and datatable-column components to avoid semantic collision with their existing grid-border model.
