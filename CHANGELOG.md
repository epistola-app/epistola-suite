...
- **Application time now flows through mediator execution context.** `MediatorContext` now binds a `MediatorExecutionContext` containing the mediator, active `Clock`, and optional principal; `SpringMediator` creates that context for direct command/query dispatch; and `EpistolaClock` resolves time from the mediator context before falling back to UTC. Immediate scheduled work binds a mediator scope with `MediatorContext.runWithMediator(mediator)`, while executor handoffs capture an explicit `MediatorExecutionContext` with `MediatorExecutionContext.capture(mediator)` and bind that context at the boundary. Obvious production JVM `now()` call sites across API responses, API keys, consumer status, assets/content storage, document generation, support snapshots/entitlements, and load-test maintenance now use `EpistolaClock`; DB-owned `NOW()/now()` paths remain on the database clock.
- **Agent guidance now documents mediator-owned application time.** `CLAUDE.md` records the required `EpistolaClock`/`MediatorExecutionContext` rules for application time, thread handoffs, database-owned time, and tests; `AGENTS.md` now directs agents to read `CLAUDE.md` before making changes.
- **Immediate scheduler scopes now use `MediatorContext.runWithMediator`.** Scheduler-style entrypoints bind mediator context directly when they execute on the current thread, while `MediatorExecutionContext.capture(...)` remains the explicit propagation object for executor handoffs.
- **Timer documentation now reflects the current mediator scope boundary.** `docs/timers.md` now describes same-thread timer and scheduled-task pollers as `MediatorContext.runWithMediator` scopes and reserves `MediatorExecutionContext.capture(...)` for executor/callback handoffs.
...
- **Clock architecture documentation.** Added [`docs/clock.md`](docs/clock.md), documenting the current `EpistolaClock` scoped clock, delegating Spring `Clock` bean, mediator execution context propagation, test clock support, and the mediator-wide clock direction.
