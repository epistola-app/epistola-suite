...
- **[dev]** feat(editor): **Inspect the effective template JSON (Leader + J).** A power-user debug affordance: the `Leader + J` shortcut opens a read-only, copyable viewer of the effective template document (`engine.doc` — the authored node/slot graph that is saved and sent to preview), kept live as you edit. It is deliberately low-discoverability — no toolbar button — but is listed in the keyboard-shortcuts help dialog (`Leader + ?`). Implemented as a self-contained `<epistola-template-json-viewer>` element that owns its own visibility and dismissal (Escape / outside-click / close), so the toolbar and editor shell carry no viewer state; the shortcut is registered in the core shortcut registry.
