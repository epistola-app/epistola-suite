...
- **[user]** feat(features): **Support-tier features default on with the support tier.** Enabling the support tier (`epistola.support.enabled=true`) now opts every support feature in by default — feedback, backups, and upgrading (alongside telemetry) — instead of each needing a separate per-feature flag. Their global default follows `epistola.support.enabled` (resolved in `FeatureToggleService`): **on when the tier is on, off in OSS deployments** where they can't work; the hub entitlement still gates actual availability when the tier is on, and per-tenant DB overrides still apply. `FeatureDefaults` no longer configures the support-tier features (the redundant `epistola.features.support-*` defaults were removed). **Behavior change:** previously `support-feedback` defaulted on even in OSS (`application.yaml`); it now follows the tier like the others, so an OSS deployment (support off) no longer shows the Support → Feedback/Backups/Upgrading features. See [`docs/feature-toggles.md`](docs/feature-toggles.md).
