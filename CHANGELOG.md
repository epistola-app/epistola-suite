...
- **QR code logo/image support**: QR code blocks now support an optional center image in `logo` mode. In the editor, users can select or remove a logo asset for a QR node, and previews resolve the configured asset content URL. Generated PDFs render QR codes with the embedded logo while preserving decodability-oriented behavior for typical payload sizes.
