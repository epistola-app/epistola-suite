...
- **Application time now flows through mediator execution context.** `MediatorContext` now binds a `MediatorExecutionContext` containing the mediator, active `Clock`, and optional principal; `SpringMediator` creates that context for direct command/query dispatch; and `EpistolaClock` resolves time from the mediator context before falling back to UTC. Scheduled work and executor handoffs now capture an explicit `MediatorExecutionContext` with `MediatorExecutionContext.capture(mediator)` and bind that context at the boundary. Obvious production JVM `now()` call sites across API responses, API keys, consumer status, assets/content storage, document generation, support snapshots/entitlements, and load-test maintenance now use `EpistolaClock`; DB-owned `NOW()/now()` paths remain on the database clock.
