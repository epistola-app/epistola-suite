...
- **Cluster scheduling is hardened against slow handlers (multi-node safety).** Three changes remove a duplicate-execution window that opened when a scheduled-task or timer handler ran longer than the cluster idle/lease timeouts (e.g. tenant backups, snapshot uploads, hub calls): (1) the node **heartbeat now runs on its own dedicated executor** (`ClusterNodeHeartbeatScheduler`) instead of sharing the default single-thread Spring `@Scheduled` pool, so a blocked handler can no longer starve liveness, falsely mark a healthy node stale, and shift routing-key ownership; (2) both pollers **renew the lease while a handler runs** via a shared `ClusterLeaseRenewer` (background thread, `renewLeases` on each registry), so a legitimately long handler is not reclaimed and re-run by another node — even across a membership change; (3) **per-`taskType`/`timerType` handler metrics** (`epistola.scheduled.task.duration` tagged `cluster-scheduled-task:<type>` / `cluster-timer:<type>`) make a single slow or failing task visible in the fleet instead of being hidden inside the poll-cycle timer. Every native scheduled-task registration now declares its `executionScope` explicitly. Adds unit coverage for the schedule calculator (backoff cap/ceiling, fixed-rate coalesce vs catch-up, cron DST/zone, error paths) and integration coverage for reclaim idempotency (a stale owner's `complete`/`fail` is a no-op) and lease renewal blocking reclaim.
