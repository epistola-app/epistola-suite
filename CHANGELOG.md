...
- **Test suite shares one Postgres container instead of one per Spring context.** `TestcontainersConfiguration.postgresContainer` was a plain per-context `@Bean`, so every distinct cached `ApplicationContext` started its own `postgres:17` container (each with a RAM-backed tmpfs data dir). Measured in `apps:epistola:test`: ~6 contexts → **6–7 concurrent containers**, which caused container-startup contention (`Container startup failed for image postgres:17`) and intermittent context-load failures (the recurring `PrometheusEndpointTest` flake). The container is now a JVM-wide `@JvmStatic` singleton started once and reaped by Ryuk at exit, wired into every context via the same `@ServiceConnection` bean — the suite already isolates tests **logically** (namespaced-tenant cleanup in `BaseIntegrationTest.resetDatabaseState`), not via a database-per-context, so one container is sufficient. Result: **1 container** (was 6–7), **0 startup failures** (was 7), **0 failed contexts** (was 1), and a faster run. The dead `.withReuse(true)` was dropped — it only helps across separate JVM runs and silently no-ops unless `testcontainers.reuse.enable` is set (it is not, in CI or by default), so it never addressed the in-JVM duplication.
