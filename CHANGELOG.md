...
- **[user]** feat(templates): **Sortable, paginated templates list.** The templates list now has clickable column headers (Name, Variants, Last Modified) to sort, numbered pagination with a rows-per-page selector (25 / 50 / 100), and a "Showing X–Y of N" summary — so the page stays usable with 100+ templates (it previously showed only the first 50, silently). Sort, search, the catalog filter and the page number all compose and live in the URL, so the view is bookmarkable and survives a refresh. It is built on a new **reusable server-rendered data-table component** in `epistola-web` (the sort + pagination chrome; each page supplies its own row markup), and every control URL is constructed **server-side** — no client-side URL assembly. The other list pages adopt the same component next. See [`docs/adr/0007-query-pagination-sorting-filtering.md`](docs/adr/0007-query-pagination-sorting-filtering.md).
