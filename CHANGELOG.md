...
- **Backend JSONata syntax validation for stencil parameter bindings ([#385](https://github.com/epistola-app/epistola-suite/issues/385)).** `NodeParameterBindingValidator` now pre-compiles every binding expression with `com.dashjoin:jsonata` at save time. Malformed expressions (e.g. unterminated string literals) are rejected with `NODE_PARAMETER_BINDING_SYNTAX_INVALID` and the parser error message is surfaced in the validation response. The template draft save route now returns these validation failures as structured `400` responses instead of generic `500`s, and the editor extracts the backend message so the toolbar/parameter dialog can show the invalid binding inline. The parameter dialog also disables Save while an expression is known to be invalid JSONata. This plugs the gap where the editor caught typos but the backend silently accepted them. The `com.dashjoin:jsonata` dependency is now declared explicitly in `:modules:epistola-core` (it was already available transitively via `:modules:generation`).
