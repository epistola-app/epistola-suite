...
- **Catalog backups + compatibility via epistola-hub (`modules/epistola-support-backups`).** A new module that snapshots all of a tenant's catalogs and syncs them to epistola-hub — the shared primitive behind backups (retained, restorable) and compatibility checks (the company side tests a tenant's catalogs against unreleased Epistola versions).
...
- **New `support-backups` feature toggle** (`KnownFeatures.SUPPORT_BACKUPS`, default off) gating the Backups + Upgrading capability per tenant (managed on the existing Features page).
- **Support → Backups and Support → Upgrading pages.** The Backups page lists a tenant's snapshots (captured time, catalog count, size, latest badge), with a "Back up now" action and a restore-with-confirmation action; the Upgrading page shows compatibility-check results fetched live from the hub, grouped by the Epistola version tested. Both are gated for visibility by the `support-backups` toggle (desktop + mobile nav under "Support") and per-action by `TENANT_SETTINGS`. When the installation isn't entitled (`HubEntitlementDeniedException`) or hasn't finished registering, the pages render an informational state instead of an error.
