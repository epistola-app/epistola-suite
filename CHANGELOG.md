...
- **Native scheduled tasks now cover singleton and all-node work.** Scheduled task definitions gained an `execution_scope`: `single_owner` keeps the existing once-per-cluster lease behavior, while `each_capable_node` stores per-node runtime state in `cluster_tasks_scheduled_node_state` so every active capable node can run its own occurrence. Business `@Scheduled` usages were migrated to native scheduled tasks: feedback retry/poll, backups, upgrading snapshots, stale job recovery, load-test cleanup, document/load-test poller wakeups, hub health checks, entitlement refresh, and installation stats. The Operations → Cluster page now shows task execution scope and per-node scheduled-task state; raw Spring scheduling remains only for cluster heartbeat and native timer/scheduled-task poller wakeups.
