...
- **[dev]** test(security): **Build now fails on hardcoded credentials in committed source.** A new `NoHardcodedSecretsTest` (in `apps/epistola`'s `app.epistola.suite.architecture`, no Docker, runs in `unitTest`) scans all `application*.yaml`/`.properties` for sensitive keys (`password`, `client-secret`, the encryption key `material`, `private-key`, …) carrying a hardcoded literal value, and scans all sources for PEM private-key blocks and AWS access-key ids. Empty values and `${ENV}` placeholders are fine; a small allowlist of clearly-marked **dev/local** profile files (`application-local.yaml`, `application-localauth.yaml`, `application-keycloak.yaml`, `application-demo.yaml`) may carry throwaway secrets. Everything else — notably the base `application.yaml` and the `prod` profile — must source secrets from the environment / a Kubernetes Secret. See [`docs/testing.md`](docs/testing.md#architecture-enforcement-tests).
