...
- **Application time now flows through mediator execution context.** `MediatorContext` now binds a `MediatorExecutionContext` containing both the mediator and the active `Clock`, `SpringMediator` creates that context for direct command/query dispatch, and `EpistolaClock` resolves time from the mediator context before falling back to UTC. `BackgroundExecutionContext` now rebinds that full context for schedulers and executor handoffs. Obvious production JVM `now()` call sites across API responses, API keys, consumer status, assets/content storage, document generation, support snapshots/entitlements, and load-test maintenance now use `EpistolaClock`; DB-owned `NOW()/now()` paths remain on the database clock.
...
- **Clock architecture documentation.** Added [`docs/clock.md`](docs/clock.md), documenting the current `EpistolaClock` scoped clock, delegating Spring `Clock` bean, background execution binding, test clock support, and the future mediator-wide clock direction.
