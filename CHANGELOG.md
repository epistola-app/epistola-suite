...
- **[dev]** refactor(assets)!: **An asset's key is text, like every other resource's.** Assets were the one catalog resource addressed by a machine identifier: `ASSET_KEY` was a UUID, so an asset's key on the wire and in template content was one too. Every other resource is addressed by a readable key and carries a UUID only as its internal identity. That was not merely inelegant — it made a catalog naming its assets the way a person would, `municipality-mark`, impossible to install, because the importer parsed the slug straight back into a UUID. Installing from a registry therefore only ever worked between two Epistola Suites, each generating UUIDs the other happened to parse. Relocation had already separated identity from address for every resource and re-keyed font faces to reference an asset by identity, which leaves `assets.id` the domain's one remaining column and this a narrow change. Existing data needs no rewriting: an asset's current key is already a UUID _string_, so every stored reference keeps resolving and content-storage paths are byte-identical. Nothing can yet create a readable key — uploads still generate one and the importers still refuse anything but a UUID, pending `AssetDto.key` on the REST surface (epistola-contract#78) so such an asset can be represented there.
