...
### Changed (Breaking)
- **Domain layer switched from v1 TemplateModel to v2 TemplateDocument**: The entire domain layer (commands, queries, services, API) now uses `TemplateDocument` (normalized node/slot graph) instead of `TemplateModel` (flat block list). `TemplateVersion.templateModel` is now typed as `TemplateDocument`. `GenerationService`, `ThemeStyleResolver`, `UpdateDraft`, `CreateVersion`, `UpdateVersion`, editor context queries, and REST API endpoints all accept/return `TemplateDocument`. Theme resolution uses the new `ThemeRef` sealed class (`ThemeRefOverride`/`ThemeRefInherit`) instead of `themeId: String?`. Document style overrides use `documentStylesOverride` (nullable) instead of `documentStyles`. Default template creation (`CreateDocumentTemplate`, `CreateVariant`, `CreateVersion`) now produces v2 node/slot graph JSON. V1 typealiases are retained for transition.
- **Generation module: Rewrite PDF renderer for v2 node/slot model**: The PDF generation module (`modules/generation`) now accepts `TemplateDocument` (v2 normalized node/slot graph) instead of `TemplateModel` (v1 recursive block tree). All `BlockRenderer` types have been replaced by `NodeRenderer` types. `BlockRendererRegistry` is now `NodeRendererRegistry` with `renderNode()` and `renderSlot()` traversal methods. `DirectPdfRenderer.render()` signature now takes `TemplateDocument`. Callers in `epistola-core` (`GenerationService`, `ThemeStyleResolver`) will need to be updated separately for the v2 model.
