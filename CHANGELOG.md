...
- **Catalog wire-format schema migrations (EF-style), framework only.** Catalog imports now run an ordered, JSON-tree migration chain over each wire part _before_ binding it to the typed protocol model, so an export produced at an older per-part `schemaVersion` can be mechanically upgraded to the current shape on import — the same discipline EF Core migrations give a database. New `app.epistola.suite.catalog.migrations` package: `CatalogSchemaMigration` (one step, `from → from+1`, pure `JsonNode` transforms for manifest and resource-detail), `MigrationContext`, and `CatalogSchemaMigrator`, which validates each part’s chain is contiguous and total at startup (Flyway-like; a malformed chain fails app start) and applies the per-part version gate. The migrator is the single chokepoint for both transports — invoked inside `CatalogClient.fetchManifest` (covering every remote consumer: install, browse, upgrade-check, fingerprint) and directly in `ImportCatalogZipHandler` for the ZIP path. **No migrations exist yet** (all parts currently have `baseline == current` in `CATALOG_PART_SCHEMAS`): a payload newer than this instance is rejected (`CatalogSchemaTooNewException`); a sub-current payload passes through and binds as-is while that part’s chain is empty (transitional — strict `TooOld` enforcement lands with the first real migration for that part). All gate exceptions extend `IllegalArgumentException`, so existing import error paths surface them as 400. Architecture and rationale in [ADR 0006](docs/adr/0006-catalog-wire-format-migrations.md); roadmap in [`docs/plans/catalog-wire-format-migrations.md`](docs/plans/catalog-wire-format-migrations.md).
