...
- **Added storage layer for v0.3 generation result collection** (V26): `generation_results` is multi-level partitioned (LIST by routing partition, 64 children, each RANGE-partitioned by `created_at` for monthly cleanup); `consumer_partition_cursors` tracks per-(tenant, consumer, partition) ack progress; `consumer_node_assignments` tracks per-(tenant, consumer, node) heartbeats for the consistent hash ring. All three tables are tenant-scoped explicitly. Backed by three repository pairs in `generation/collect/persistence/` — internal helpers used by the command/query handlers (commands/queries are the tested surface).
- **Added v0.3 generation result collection end-to-end**:
  - `Partition.kt` (hard-coded `TOTAL_PARTITIONS = 64`, MurmurHash3 x86 32-bit with seed 0, byte-for-byte parity with the contract's helper).
  - `ConsistentHashRing.kt` (K=128 virtual nodes per real node) for partition-to-node assignment with the consistent-hashing property: ~1/N partitions move on join/leave, preserving submitting-node affinity through rebalances.
  - Three CQRS commands — `EmitGenerationResult` (SystemInternal, called by `DocumentGenerationExecutor` on terminal state), `TouchConsumerNode` and `AcknowledgeGenerationResults` (RequiresPermission DOCUMENT_GENERATE).
  - Two CQRS queries — `FetchGenerationResults` and `GetServerInfo`.
  - `NdjsonResultStream` (gzip-negotiated streaming writer for the wire NDJSON format).
  - REST endpoints: `EpistolaSystemApi` (POST /api/ping) and the previously-stubbed `collectGenerationResults` on `EpistolaDocumentGenerationApi` is now real (POST /api/tenants/{tenantId}/generation/collect).
  - `DocumentGenerationExecutor` and the test `FakeDocumentGenerationExecutor` both dispatch `EmitGenerationResult` after a request reaches a terminal state.
  - `PartitionMaintenanceScheduler` extended with multi-level (LIST × RANGE) maintenance: bootstraps current and next-month sub-partitions for all 64 LIST children at startup, drops sub-partitions older than retention. New property `epistola.partitions.generation-results-retention-months` (default 1).
  - `ClientIdentityFilter` registered on `/api/**` enforces the contract's `User-Agent: epistola-contract/<version>` and `X-EP-Node-Id` headers on `/ping` and `/generation/collect` (400 when absent), warn-only elsewhere.
  - New properties: `epistola.collect.idle-timeout-ms` (default 60000), `epistola.collect.api-version` (default "0.3.0"), `epistola.partitions.generation-results-retention-months` (default 1).
  - **Test scenarios for the collect lifecycle**: Six `*ScenarioIT` classes under `modules/epistola-core/src/test/.../generation/collect/scenarios/` driven by the `ControllableDocumentGenerationExecutor` + `SimulatedConsumer` harness from Step A. Cover submitting-node affinity (`routingKeyToMe` round-trip), multi-consumer independence on the same tenant, rebalance redelivery when a node goes silent (orphaned-result + unacked-row paths), unacked rows being redelivered on the next poll, partial ack advancing the cursor by partition, global sequence ordering across partitions even with out-of-order completion, and a tagged `@Tag("stress")` 100-jobs/4-nodes coverage check. Tests dispatch commands/queries via the mediator (the controller is trivial pass-through); the JobPoller is disabled via `epistola.generation.polling.enabled=false` so transitions are deterministic and the harness drives every state change.
  - **Inlined the three collect repositories**: SQL for `generation_results`, `consumer_partition_cursors`, and `consumer_node_assignments` now lives directly in `EmitGenerationResultHandler`, `AcknowledgeGenerationResultsHandler`, `TouchConsumerNodeHandler`, and `FetchGenerationResultsHandler` — matches the suite-wide CQRS-only convention (no separate persistence layer for the documents/, tenants/, themes/ domains either). Deleted the six files under `generation/collect/persistence/`. `FetchGenerationResults` now reads cursors and rows in a single transaction, closing the small race where a concurrent ack could move the cursor between the two reads. Tests that previously autowired the repositories now use raw `Jdbi` for setup/inspection or call the new handlers via the mediator.
  - **Flattened `generation_results` partitioning** (V27): dropped the multi-level LIST(partition) × RANGE(created_at) shape introduced in V26 in favor of a single-level RANGE(created_at) layout. The original two-tier shape created 256 sub-partitions per year, required a regex-driven maintenance path, a `DO $$` bootstrap for every LIST child, and exposed a "missing sub-partition" failure mode at month boundaries. Partition-filtered reads were already pruned by the `(partition, sequence)` index on the parent — the LIST level wasn't earning its complexity. `Partition.partitionFor(...)` and the `partition` column are unchanged; only the physical storage differs. `PartitionMaintenanceScheduler` is correspondingly simpler — it now treats `generation_results` as a normal RANGE table with its own retention setting (`epistola.partitions.generation-results-retention-months`, still defaulting to 1), and the multi-level helpers (`MultiLevelPartitionConfig`, `createRequiredMultiLevelPartitions`, `dropOldMultiLevelPartitions`, `createSubPartitionForMonth`, `parseMonthSuffix`) are gone. New `PartitionMaintenanceSchedulerIT` covers the bootstrap + idempotency + retention sweep path on the flat shape.
  - **HTTP-level smoke for the v0.3 collect surface** (`CollectEndpointSmokeIT`): boots `EpistolaSuiteApplication` on a random port and exercises `/api/ping` and `/api/tenants/{id}/generation/collect` over real HTTP with a real API key, verifying the bits the mediator-driven scenario tests can't see — Spring routing wiring, `ClientIdentityFilter` rejecting missing `X-EP-Node-Id`/malformed `User-Agent` with 400, `ApiKeyAuthenticationFilter` returning 401 for unauthenticated callers, the contract media types (`application/vnd.epistola.v1+json` and `application/vnd.epistola.v1+ndjson`) negotiating correctly, and the NDJSON `_meta` line shape that crosses the wire. The smoke also verifies `Accept-Encoding: identity` negotiates uncompressed responses (gzip is the default).

- **Pulled the `api-key` `authMethod` enum value into the v0.3 contract** ahead of the suite-side Consumer Management implementation. The contract's `ConsumerDto.authMethod` was previously `[oauth, self-signed-jwt]`; api-key was the planned third value but would have required its own contract bump down the line. Since v0.3.0 isn't released yet, baking it in now means the upcoming suite implementation can extend the contract with `getMyConsumer` only, without also having to add this enum value. Generated client + server expose `API_KEY("api-key")`. The `POST /consumers/register` description also explicitly carves out api-key consumers (provisioned out of band by tenant managers, materialize directly as `active`). See `docs/consumer-management.md`.
- **Captured Consumer Management API design** in `docs/consumer-management.md` — the contract spec already defines a complete consumer lifecycle (`/tenants/{tid}/consumers` with `register`/`approve`/`reject`/`update`/`delete`/`rotatePublicKey`, rich `ConsumerDto` including `nodes: ConsumerNodeDto[]` with `lastSeenAt`, partitions, and userAgent). The suite has not implemented any of it yet — operator visibility into "which consumer / which nodes are connected to my tenant" goes through that surface naturally. Phased plan covers the read-only model + cluster-view UI in both the suite (Thymeleaf/HTMX) and the Valtimo plugin (Angular admin tab) first, then the lifecycle endpoints, then fine-grained permission enforcement. Not a v0.3 deliverable; tracked as a separate work stream with its own contract bump.
- **Documented v0.3 affinity-during-rebalance behavior** in `docs/v04-coordinated-rebalance.md` and on the contract `PartitionAssignment` schema. In v0.3, when a new consumer node joins, partition ownership rebalances immediately on the server but existing nodes only learn about it on their next poll (≤ 1 s at default `minInterval`). During that brief window an existing node's cached `routingKeyToMe(...)` may pick a partition the new node now owns; the result still flows (at-least-once delivery is preserved) but is collected by the new owner instead of the submitter. Coordinated cutover with a `pending` assignment + wall-clock `cutoverAt` is designed for v0.4 — see the doc.
