...
- **Display utility CSS classes.** Added `ep-hidden`, `ep-inline`, `ep-inline-block`, `ep-inline-flex` classes.
...
- **Dynamic CSS utilities for th:style-powered computed values.** Added `.ep-depth-indent` (tree-indent via `--ep-depth * 16px`), `.ep-opacity-half` (generic `opacity: 0.5`), and `--ep-bar-width` support on `.progress-bar` (custom property width).

- **Page-specific and extended utility CSS classes (Group 7).** Added width utilities (`ep-w-auto`, `ep-w-full`, `ep-max-w-300`), extended flex utilities (`ep-flex-shrink-0`, `ep-flex-start`, `ep-flex-end`, `ep-justify-end`), overflow (`ep-overflow-x-auto`), cursor (`ep-cursor-pointer`), opacity (`ep-opacity-60`, `ep-opacity-70`), color (`ep-text-red`, `ep-text-green`), fieldset reset (`ep-fieldset-reset`), and word-break (`ep-word-break-all`) utilities. Added component-level CSS for contract status bar, feedback code blocks, feedback success dialog, catalog import tag, catalog version status, comparison dialog body, PDF/A toggle, stencil upgrade select, table checkbox column, pagination bar, screenshot preview, and dialog width overrides.
...
- **Inline style enforcement test.** Added `InlineStyleHygieneTest` with 8 checks that fail CI on any new violations: HTML inline `style="..."`, Thymeleaf `th:style` without `--ep-*`, duplicate `class` attributes, CSS `var(--ep-*, <fallback>)` in source files, TypeScript static `style="..."`, TypeScript `var(--ep-*, <fallback>)` string literals, TypeScript runtime `style.color/background` assignments (with exemption for content-driven ProseMirror colors), and TypeScript `classList.add/remove/toggle/contains`. Enforces the data-attribute-first pattern: no inline styles, no classList for state-driven styling, no CSS token fallbacks.

### Changed

- **Replaced inline spacing styles with utility classes across 30 templates.** Pure margin/padding `style="..."` attributes swapped for `ep-mt-*`, `ep-mb-*`, `ep-ml-*`, `ep-mr-*`, `ep-p-*`, `ep-pl-*` classes.

- **Replaced inline typography styles with utility classes across 31 templates.** `font-weight`, `font-size`, `font-family`, `text-align`, `white-space`, `text-decoration` inline styles swapped for utility classes.

- **Replaced inline display styles with utility classes and migrated JS-driven visibility toggles.** `display: inline` on `<form>` elements replaced with `ep-inline`; `display: none` elements (toggled by JS) replaced with `ep-hidden` class and `element.style.display` assignments migrated to `classList.add`/`remove`/`toggle`. Affects 15 template files.

- **Replaced inline flex layout styles with utility classes across 18 template files.** `display: flex`, `justify-content`, `align-items`, `gap`, `flex-direction`, `flex-wrap`, and `flex` properties replaced with `ep-flex`, `ep-flex-center`, `ep-flex-between`, `ep-flex-col`, `ep-flex-wrap`, `ep-flex-1`, `ep-flex-2`, and `ep-gap-{1,2,3,4}` classes.

- **Replaced error page inline styles with component classes in 4 error page templates.** Body, card, and message patterns in `403.html`, `404.html`, `500.html`, `error.html` replaced with `.error-page-body`, `.error-page-card`, `.error-page-message` classes.

- **Replaced dynamic `th:style` expressions with class-based alternatives across 4 templates.** Two conditional `th:style` (display:none on attribute editor rows, opacity on subscribed stencils) migrated to `th:classappend` with `ep-hidden`/`ep-opacity-half`. Two computed `th:style` (tree-indent padding in data contract schema, progress bar width in loadtest detail) migrated to CSS custom property `th:style` + CSS class, moving the visual factor (16px, percentage) from the template to the stylesheet.

- **Replaced page-specific inline styles with CSS classes across 28 template files.** Removed ~52 inline `style="..."` attributes spanning catalog-filter selects, fieldset resets, dialog widths, feedback code blocks, contract status bar, comparison dialog body, pagination bars, PDF/A toggle, screenshot previews, login buttons, catalog version status badges, and many more. All visual values preserved exactly — presentation moved from inline attributes to named CSS classes.

- **Replaced the remaining app utility-class usage with contextual CSS across the app.** The follow-up cleanup removed the temporary utility layer from `main.css` (`ep-hidden`, `ep-inline`, spacing/typography/width/flex helpers, and similar one-off app utilities) and updated the last templates/scripts that still depended on it to use page-scoped selectors, named classes, or `data-hidden="true|false"` toggles instead. Also moved the Consumers dashboard styles out of its template-local `<style>` block into `main.css`, leaving the app stylesheet organized around concrete components and pages instead of a parallel utility system.
...
- **Replaced all `classList.add/remove/toggle` state-driven styling with HTML data attributes across 8 editor TypeScript files.** Migrated 52 `classList` calls to `dataset.*` + CSS `&[data-*]` nested selectors. Affected files: `stencil-picker-dialog.ts` (step visibility, card selection, disabled state), `binding-row.ts` (validation valid/invalid states, error visibility), `parameter-bindings-dialog.ts` (error visibility), `asset-picker-dialog.ts` (asset selection, upload/drag states), `table-dialog.ts` (cell selection), `bubble-menu.ts` (active state), `ExpressionNodeView.ts` (raw/resolved display), `EpistolaCanvas.ts` (dragging/drag-over states), `EpistolaPalette.ts` (dragging), `EpistolaTree.ts` (dragging). CSS files updated with `&[data-*='true']` nested inside the base class: `stencil.css`, `image.css`, `table.css`, `prosemirror.css`, `dnd.css`, `clean-mode.css`. Also removed remaining `style.color` runtime assignments from `binding-row.ts` and `inline-rich-text-rendering.ts` (ProseMirror content-driven colors exempted via exemption clause). All state now lives in explicit data attributes, visuals in CSS — no JavaScript directly manipulates presentation.
