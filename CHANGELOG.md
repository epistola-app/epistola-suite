...
- **Locale-aware date formatting in the editor preview (Phase 2 foundation).** `$formatDate(value, 'd MMMM yyyy')` in the editor's inline expression preview now honours a locale argument: month-name tokens (`MMMM`, `MMM`) are resolved via `Intl.DateTimeFormat` instead of a hardcoded English array, with the per-locale month names cached so the cost is paid once. Numeric tokens (`yyyy`, `MM`, `dd`, etc.) remain locale-agnostic, matching the Java `DateTimeFormatter` semantics on the renderer side. The exported `formatDateValue` / `evaluateExpression` / `tryEvaluateExpression` functions gain an optional `locale` parameter (defaults to `"en-US"`, so existing call sites are unchanged). The locale-chain consumer wiring (variant attribute → tenant default → app default) is in place via the extended `TenantLocaleResolver.resolve(tenant, variantAttributes)` and `resolveLocale(...)`; threading that into the four editor previews (`ExpressionNodeView`, `EpistolaRichTextVariablePreview`, `EpistolaQrCodePreview`, `parameter-evaluation-cache`) and into `DirectPdfRenderer.render` lands in the next commit. Canonical case: `formatDateValue('2026-04-04', 'dd MMMM yyyy', 'nl-NL')` now yields `"04 april 2026"` (Dutch lowercase month per CLDR).
