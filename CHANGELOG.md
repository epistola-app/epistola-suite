...
- **Complete catalog resource types**: Catalogs now support themes, stencils, attributes, and assets as first-class resources alongside templates. Importing a catalog installs all resource types in dependency order (assets → attributes → themes → stencils → templates). New `catalog_resources` table replaces `catalog_templates` for generic resource tracking. Includes polymorphic Jackson deserialization via sealed `CatalogResource` hierarchy, per-type import commands (`ImportTheme`, `ImportStencil`, `ImportAttribute`, `ImportAsset`), export queries for all resource types, binary content fetching for assets, and a `DependencyScanner` that auto-includes referenced themes, stencils, and assets when exporting templates. The `ExportCatalog` command builds self-contained catalog manifests with all dependencies resolved. Schema version bumped to 2.
