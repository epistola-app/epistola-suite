...
- **[user]** feat(themes): **The themes list is now sortable, paginated and searchable.** The themes list gains clickable column headers (Name, Last Modified) to sort, numbered pagination with a rows-per-page selector (10 / 25 / 50) and a "Showing X–Y of N" summary, a search box, and a catalog filter. Sort, search, the catalog filter and the page number all compose and live in the URL, so the view is bookmarkable and survives a refresh — changing any one preserves the others. Themes is the first page on a new reusable data-table toolkit; the other list pages follow. See [`docs/adr/0007-query-pagination-sorting-filtering.md`](docs/adr/0007-query-pagination-sorting-filtering.md).
- **[dev]** feat(web): **Reusable server-rendered data-table list toolkit.** New surface-agnostic paging primitives in `epistola-core` (`PageRequest`, `PagedResult`, `SortSpec`, and a per-query `SortWhitelist` that maps logical sort keys to fixed SQL so `ORDER BY` never takes user input) plus a `pagedQuery` helper that returns one windowed page and the full filtered `total` in a single `COUNT(*) OVER()` round trip and clamps a stale out-of-range page to the last page. On top, an `epistola-web` data-table component: `ListViewState` parses and clamps the untrusted query string once at the UI boundary, `ListQuery` builds the server-side sort/page links, and the `data-table` / `list-page` / `sortable-header` / `catalog-filter` Thymeleaf fragments render the chrome. Offsets are computed in 64-bit so a large page can't overflow to a negative SQL `OFFSET`. Applied to the themes list as the reference page (UI + MCP + REST). See [`docs/adr/0007-query-pagination-sorting-filtering.md`](docs/adr/0007-query-pagination-sorting-filtering.md).
