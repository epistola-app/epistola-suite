...
Hardens credential handling: stored credentials (catalog/code-list auth, hub credentials) are now encrypted at rest with AES-256-GCM and support no-downtime key rotation, and committed secrets are caught by gitleaks at commit time and in CI.

- **[user]** feat(observability): **Forward application logs and metrics to epistola-hub over OTLP (ADR 0006).** A new `epistola-support-telemetry` module ships a dedicated, isolated OpenTelemetry (OTLP) leg that forwards application-log events (a `TelemetryLogAppender` → OTLP/HTTP) and metrics (a dedicated Micrometer `OtlpMeterRegistry` added to the composite) to a hub-operated collector — separate from any bring-your-own OTel agent / `management.otlp.*` self-export leg so the two never double-export. It is **off by default** and gated **globally (per installation)**, not per tenant: on top of `epistola.support.telemetry.enabled=true`, it activates only when the installation holds an installation-wide `support-telemetry` entitlement (a new `SupportEntitlementService.isInstallationEntitled` check; no per-tenant feature toggle). The OTLP endpoint is resolved by the support module from the hub endpoint (which proxies OTLP for now; override with `epistola.support.hub.telemetry-endpoint` once a dedicated collector exists). A tenant-scoped DENY excludes that tenant's logs (the producer-side control point in the appender), and the per-tenant `tenant` metric tag is stripped before forwarding by default for data residency. Log forwarding is fail-open (the OTLP batch processor batches, gzip-compresses, and drops on overflow), matching the local log pipeline's posture. Credentials (the installation API key) ride the existing hub registration; the hub OTLP collector that authenticates the installation and enforces entitlement is separate infrastructure. See [`docs/adr/0006-shipping-logs-and-metrics-to-hub.md`](docs/adr/0006-shipping-logs-and-metrics-to-hub.md), [`docs/metrics.md`](docs/metrics.md), and [`docs/application-logs.md`](docs/application-logs.md).
