...
- **[dev]** feat(catalog): **Example catalog wire-format migrations — the chain now spans `[3, 5]`.** Two reference migration steps land (`CATALOG_BASELINE_SCHEMA_VERSION = 3`, `CATALOG_SCHEMA_VERSION = 5`): `CatalogV3ToV4ExampleMigration` upgrades a `v3` catalog — legacy `catalog.title` / `resource.displayName` fields — to `v4` (`name`) on import, defensively and idempotently; `CatalogV4ToV5Migration` is the simplest possible migration, a **no-op version bump** (`4 → 5` with no content change, overriding neither transform — the migrator just re-stamps `schemaVersion`). Together they are the **reference implementations** for writing a real migration (declare `from`, optionally override `migrateManifest` / `migrateResourceDetail`, register as a `@Component`, bump the version). Unit tests cover each step and the end-to-end gate; committed bundled/test catalogs stay at `v4` (within the `[3, 5]` window — migrated to `5` on load, fingerprints unchanged since `schemaVersion` is excluded).
